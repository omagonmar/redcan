      SUBROUTINE JDBVIP(md,ncp,ndp,xd,yd,zd,nip,xi,yi,zi,dqi,iwk, 
     :                  wk,istat,status)
...
c     dqi = array of dimension nip where output mask values
c           are to be stored (0=within bounds; 1=outside bounds)
...
c     :          zi(1000), iwk(4100), wk(800)
      dimension xd(*), yd(*), zd(*), xi(*), yi(*), zi(*), dqi(*)
      dimension iwk(*), wk(*)
      common /idlc  / nit
      common /idpi  / itpv
      integer istat
...
c interpolates the zi values.  (for md=1,2,3)
                  ...
                  do 5 iip = 1, nip0
                     jwit = jwit + 1
                     call PDA_JDPTIP(xd,yd,zd,nt,iwk(jwipt),nl, 
