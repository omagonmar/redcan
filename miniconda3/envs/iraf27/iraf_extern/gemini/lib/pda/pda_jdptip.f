      SUBROUTINE PDA_JDPTIP(xd,yd,zd,nt,ipt,nl,ipl,pdd,iti,xii, 
     :                      yii,zii,dqii,istat)
 
c this subroutine performs punctual interpolation or extrapola-
c tion, i.e., determines the z value at a point.
c
c based on PDA_IDPTIP, with changes by James to provide a data quality
c output array as well as the interpolation results
...
c     dqii = mask value (0=interpolated, 1=out of bounds). (JT)
...
c In April 2009, I also got around to setting mask array (DQ) values
c based on whether interpolating or setting data outside bounds to 0.
         zii = 0.0
         dqii = 1
         return
