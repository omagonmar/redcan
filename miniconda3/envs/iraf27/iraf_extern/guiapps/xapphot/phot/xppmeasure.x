...
# XP_PLMEAS -- Procedure to execute the sky fitting task measuring commands.

pointer procedure xp_plmeas (gd, xp, im, ol, rl, gl, key, wx, wy, oxver,
...
begin
	...
	case 'o', '-', '.':
		...
		call gdeactivate (gd, 0)
                call xp_pqprint (xp, Memc[imname], cier, sier, pier,
		    LOGRESULTS(pstatus))
	...
	case '+', '#':
		...
		nobjs = 0
		    ...
		    nobjs = nobjs + 1
                ...
                if (rl != NULL && LOGRESULTS(pstatus) == YES) {
                    call printf (
		    "Wrote %d detected and measured list objects to file %s\n")
                        call pargi (nobjs)
                        call pargstr (Memc[rlname])
                } else {
                    call printf ("Detected and measured %d list objects\n")
                        call pargi (nobjs)
                }
                object = EOF
                NEWCBUF(pstatus) = NO; NEWCENTER(pstatus) = NO
...
# XP_PMEAS -- Procedure to execute the sky fitting task measuring commands.

pointer procedure xp_pmeas (gd, xp, im, rl, gl, key, wx, wy, oxver,
	oyver, nover, sxver, syver, nsver)
...
int	xp_stati(), clgcur(), xp_cfit1(), xp_sfit1(), xp_mag1(), xp_aphot()
pointer	xp_statp()
real	xp_statr()
...
begin
	    ...
	    # Measure.
		    ...
		    (xp_stati(xp,SGEOMETRY) == XP_SPOLYGON && nsver < 3)) {
			    ...
			    gd, NEWSBUF(pstatus), NEWSKY(pstatus))
            		...
            		if (xp_stati(xp, SALGORITHM) != XP_CONSTANT &&
			    xp_stati(xp, SALGORITHM) != XP_ZERO &&
			    xp_stati(xp,SALGORITHM) != XP_SKYFILE) {
                		call printf ("Again for sky:\n")
                		if (clgcur ("gcommands", swx, swy, wcs,
				    key, Memc[str], SZ_LINE) == EOF)
                    		    ;
            		}
	...
	case '*':
            if (rl != NULL && LOGRESULTS(pstatus) == YES) {
                nobjs = xp_aphot (gd, im, rl, xp, NO)
                call printf (
		    "Wrote %d detected and measured objects to file %s\n")
                    call pargi (nobjs)
                    call pargstr (Memc[rlname])
            } else {
                nobjs = xp_aphot (gd, im, NULL, xp, NO)
