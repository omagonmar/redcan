...
int	i, j, nc, nl, open(), imgnls(), imloop()
...
begin
	...
	if (nopen >= MAX_OPENIM) {
		...
		do i = min_open, nalloc {
		    ...
		    nl = XT_BUFSIZE(xt1) / pix_size[TY_SHORT] / IM_LEN(im,1)
		    if (nl > 1) {
			nc = IM_LEN(im,1)
...
int	i, j, nc, nl, open(), imgnli(), imloop()
...
begin
	...
	if (nopen >= MAX_OPENIM) {
		...
		do i = min_open, nalloc {
		    ...
		    # Buffer some number of lines.
		    nl = XT_BUFSIZE(xt1) / pix_size[TY_INT] / IM_LEN(im,1)
		    if (nl > 1) {
			nc = IM_LEN(im,1)
			call amovl (v, XT_VS(xt1,1), IM_MAXDIM)
			call amovl (v, XT_VE(xt1,1), IM_MAXDIM)
...
int	i, j, nc, nl, open(), imgnlr(), imloop()
...
begin
	...
	if (nopen >= MAX_OPENIM) {
		...
		do i = min_open, nalloc {
		    ...
		    # Buffer some number of lines.
		    nl = XT_BUFSIZE(xt1) / pix_size[TY_REAL] / IM_LEN(im,1)
		    if (nl > 1) {
			nc = IM_LEN(im,1)
			call amovl (v, XT_VS(xt1,1), IM_MAXDIM)
			call amovl (v, XT_VE(xt1,1), IM_MAXDIM)
...
int	i, j, nc, nl, open(), imgnld(), imloop()
...
begin
	...
	if (nopen >= MAX_OPENIM) {
		...
		do i = min_open, nalloc {
		    ...
		    # Buffer some number of lines.
		    nl = XT_BUFSIZE(xt1) / pix_size[TY_DOUBLE] / IM_LEN(im,1)
		    if (nl > 1) {
			nc = IM_LEN(im,1)
			call amovl (v, XT_VS(xt1,1), IM_MAXDIM)
			call amovl (v, XT_VE(xt1,1), IM_MAXDIM)
