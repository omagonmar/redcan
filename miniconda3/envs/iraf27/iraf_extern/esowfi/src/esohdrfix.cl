...
#
#
# Modified to deal with OUTn and CHIPn keywords. This makes this script
# specific to the ESO WFI.



procedure esohdrfix (input, output)
...
begin
	...
	while (fscan (fd, line) != EOF) {
	    ...
	    if (substr (key, 1, 4) == "CHIP") {
		if (substr (key, 5, 5) != "S")
		    key = substr (key, 1, 4) // substr (key, 6, 1000)
	    } else if (substr (key, 1, 3) == "OUT") {
		key = substr (key, 1, 3) // substr (key, 5, 1000)
	    }
