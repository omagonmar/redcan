...
procedure im_sum_i(ip_1,weight1,ip_2,weight2,ip_sum)
pointer ip_1	# i: first input image
int	weight1 # i: weight for first image
pointer ip_2    # i: second input image
int	weight2 # i: weight for second image
...
pointer imgnli()	# returns pointer to input image data [sys/imio]
pointer impnli()	# returns pointer to output image data [sys/imio]
...
begin
	...
	# Assign p_1row to be pointer to row of first image.
        #---------------------------------------------------
        while (imgnli(ip_1,p_1row,v1)!=EOF)
        {
           #---------------------------------------------------
           # Get pointer to row of second image
           #---------------------------------------------------
           if (imgnli(ip_2,p_2row,v2)==EOF)
           {
	   ...
	   #  should go.)
           #---------------------------------------------------
           if (impnli(ip_sum,p_sumrow,vsum)==EOF)
           {
           ...
           #---------------------------------------------------
            if (!ignore_weight1)
           {
              call amulki(Memi[p_1row],weight1,Memi[p_1row],row_length)
           }
           ...
           if (!ignore_weight2)
           {
             call amulki(Memi[p_2row],weight2,Memi[p_2row],row_length)
           }

           #---------------------------------------------------
           # Add weighted rows to row from output image
           #---------------------------------------------------
           call aaddi(Memi[p_1row],Memi[p_2row],Memi[p_sumrow],row_length)
        }
...
procedure im_cumul_i(ip_1,ip_sum)
...
begin
	...
	# Assign p_1row to be pointer to row of first image.
        ...
        while (imgnli(ip_1,p_1row,v1)!=EOF)
        {
           ...
           #-----------------------------------------------------
           if (imgnli(ip_sum,p_sumrow,vsum_in)==EOF)
           {
              call error(PROS_WRONG_SIZE,
		"Cumulative image has fewer rows than first image.")
           ...
           #---------------------------------------------------
           call aaddi(Memi[p_1row],Memi[p_sumrow],
					Memi[p_temprow],row_length)

           #-----------------------------------------------------
           # Get pointer to row of final image (to be written to)
           #-----------------------------------------------------
           if (impnli(ip_sum,p_sumrow,vsum_out)==EOF)
           {
              call error(PROS_UNEXPECTED_EOF,
			"EOF reached on writing output image.")
           }
           ...
           #---------------------------------------------------
	   call amovi(Memi[p_temprow],Memi[p_sumrow],row_length)
	}
...
begin
        ...
        #---------------------------------------------------
        while (impnli(ip,p_row,v)!=EOF)
        {
           #---------------------------------------------------
           # Clear the values in the row.
           #---------------------------------------------------
           call aclri(Memi[p_row],row_length)
        }
end     
