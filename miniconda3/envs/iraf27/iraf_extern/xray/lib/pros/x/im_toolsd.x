...
procedure im_sum_d(ip_1,weight1,ip_2,weight2,ip_sum)
...
pointer imgnld()	# returns pointer to input image data [sys/imio]
pointer impnld()	# returns pointer to output image data [sys/imio]
...
begin
	...
	# Assign p_1row to be pointer to row of first image.
        #---------------------------------------------------
        while (imgnld(ip_1,p_1row,v1)!=EOF)
        {
           #---------------------------------------------------
           # Get pointer to row of second image
           #---------------------------------------------------
           if (imgnld(ip_2,p_2row,v2)==EOF)
           {
	   ...
	   #  should go.)
           #---------------------------------------------------
           if (impnld(ip_sum,p_sumrow,vsum)==EOF)
           {
           ...
           #---------------------------------------------------
            if (!ignore_weight1)
           {
              call amulkd(Memd[p_1row],weight1,Memd[p_1row],row_length)
           }
           ...
           if (!ignore_weight2)
           {
             call amulkd(Memd[p_2row],weight2,Memd[p_2row],row_length)
           }

           #---------------------------------------------------
           # Add weighted rows to row from output image
           #---------------------------------------------------
           call aaddd(Memd[p_1row],Memd[p_2row],Memd[p_sumrow],row_length)
        }
...
procedure im_cumul_d(ip_1,ip_sum)
...
begin
	...
	# Assign p_1row to be pointer to row of first image.
        ...
        while (imgnld(ip_1,p_1row,v1)!=EOF)
        {
           ...
           #-----------------------------------------------------
           if (imgnld(ip_sum,p_sumrow,vsum_in)==EOF)
           {
              call error(PROS_WRONG_SIZE,
		"Cumulative image has fewer rows than first image.")
           ...
           #---------------------------------------------------
           call aaddd(Memd[p_1row],Memd[p_sumrow],
					Memd[p_temprow],row_length)

           #-----------------------------------------------------
           # Get pointer to row of final image (to be written to)
           #-----------------------------------------------------
           if (impnld(ip_sum,p_sumrow,vsum_out)==EOF)
           {
              call error(PROS_UNEXPECTED_EOF,
			"EOF reached on writing output image.")
           }
           ...
           #---------------------------------------------------
	   call amovd(Memd[p_temprow],Memd[p_sumrow],row_length)
	}
...
begin
        ...
        #---------------------------------------------------
        while (impnld(ip,p_row,v)!=EOF)
        {
           #---------------------------------------------------
           # Clear the values in the row.
           #---------------------------------------------------
           call aclrd(Memd[p_row],row_length)
        }
end     
