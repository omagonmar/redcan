...
procedure im_sum_r(ip_1,weight1,ip_2,weight2,ip_sum)
pointer ip_1	# i: first input image
real	weight1 # i: weight for first image
pointer ip_2    # i: second input image
real	weight2 # i: weight for second image
...
pointer imgnlr()	# returns pointer to input image data [sys/imio]
pointer impnlr()	# returns pointer to output image data [sys/imio]
...
begin
	...
	# Assign p_1row to be pointer to row of first image.
        #---------------------------------------------------
        while (imgnlr(ip_1,p_1row,v1)!=EOF)
        {
           #---------------------------------------------------
           # Get pointer to row of second image
           #---------------------------------------------------
           if (imgnlr(ip_2,p_2row,v2)==EOF)
           {
	   ...
	   #  should go.)
           #---------------------------------------------------
           if (impnlr(ip_sum,p_sumrow,vsum)==EOF)
           {
           ...
           #---------------------------------------------------
            if (!ignore_weight1)
           {
              call amulkr(Memr[p_1row],weight1,Memr[p_1row],row_length)
           }
           ...
           if (!ignore_weight2)
           {
             call amulkr(Memr[p_2row],weight2,Memr[p_2row],row_length)
           }

           #---------------------------------------------------
           # Add weighted rows to row from output image
           #---------------------------------------------------
           call aaddr(Memr[p_1row],Memr[p_2row],Memr[p_sumrow],row_length)
        }
...
procedure im_cumul_r(ip_1,ip_sum)
...
begin
	...
	# Assign p_1row to be pointer to row of first image.
        ...
        while (imgnlr(ip_1,p_1row,v1)!=EOF)
        {
           ...
           #-----------------------------------------------------
           if (imgnlr(ip_sum,p_sumrow,vsum_in)==EOF)
           {
              call error(PROS_WRONG_SIZE,
		"Cumulative image has fewer rows than first image.")
           ...
           #---------------------------------------------------
           call aaddr(Memr[p_1row],Memr[p_sumrow],
					Memr[p_temprow],row_length)

           #-----------------------------------------------------
           # Get pointer to row of final image (to be written to)
           #-----------------------------------------------------
           if (impnlr(ip_sum,p_sumrow,vsum_out)==EOF)
           {
              call error(PROS_UNEXPECTED_EOF,
			"EOF reached on writing output image.")
           }
           ...
           #---------------------------------------------------
	   call amovr(Memr[p_temprow],Memr[p_sumrow],row_length)
	}
...
begin
        ...
        #---------------------------------------------------
        while (impnlr(ip,p_row,v)!=EOF)
        {
           #---------------------------------------------------
           # Clear the values in the row.
           #---------------------------------------------------
           call aclrr(Memr[p_row],row_length)
        }
end     
