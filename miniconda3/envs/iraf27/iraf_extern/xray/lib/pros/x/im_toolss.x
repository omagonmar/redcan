...
procedure im_sum_s(ip_1,weight1,ip_2,weight2,ip_sum)
pointer ip_1	# i: first input image
short	weight1 # i: weight for first image
pointer ip_2    # i: second input image
short	weight2 # i: weight for second image
...
pointer imgnls()	# returns pointer to input image data [sys/imio]
pointer impnls()	# returns pointer to output image data [sys/imio]
...
begin
	...
	# Assign p_1row to be pointer to row of first image.
        #---------------------------------------------------
        while (imgnls(ip_1,p_1row,v1)!=EOF)
        {
           #---------------------------------------------------
           # Get pointer to row of second image
           #---------------------------------------------------
           if (imgnls(ip_2,p_2row,v2)==EOF)
           {
	   ...
	   #  should go.)
           #---------------------------------------------------
           if (impnls(ip_sum,p_sumrow,vsum)==EOF)
           {
           ...
           #---------------------------------------------------
            if (!ignore_weight1)
           {
              call amulks(Mems[p_1row],weight1,Mems[p_1row],row_length)
           }
           ...
           if (!ignore_weight2)
           {
             call amulks(Mems[p_2row],weight2,Mems[p_2row],row_length)
           }

           #---------------------------------------------------
           # Add weighted rows to row from output image
           #---------------------------------------------------
           call aadds(Mems[p_1row],Mems[p_2row],Mems[p_sumrow],row_length)
        }
...
procedure im_cumul_s(ip_1,ip_sum)
...
begin
	...
	# Assign p_1row to be pointer to row of first image.
        ...
        while (imgnls(ip_1,p_1row,v1)!=EOF)
        {
           ...
           #-----------------------------------------------------
           if (imgnls(ip_sum,p_sumrow,vsum_in)==EOF)
           {
              call error(PROS_WRONG_SIZE,
		"Cumulative image has fewer rows than first image.")
           ...
           #---------------------------------------------------
           call aadds(Mems[p_1row],Mems[p_sumrow],
					Mems[p_temprow],row_length)

           #-----------------------------------------------------
           # Get pointer to row of final image (to be written to)
           #-----------------------------------------------------
           if (impnls(ip_sum,p_sumrow,vsum_out)==EOF)
           {
              call error(PROS_UNEXPECTED_EOF,
			"EOF reached on writing output image.")
           }
           ...
           #---------------------------------------------------
	   call amovs(Mems[p_temprow],Mems[p_sumrow],row_length)
	}
...
begin
        ...
        #---------------------------------------------------
        while (impnls(ip,p_row,v)!=EOF)
        {
           #---------------------------------------------------
           # Clear the values in the row.
           #---------------------------------------------------
           call aclrs(Mems[p_row],row_length)
        }
end     
