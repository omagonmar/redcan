# $Header: /home/pros/xray/xdataio/fits2qp/RCS/fits_rheader.x,v 11.0 1997/11/06 16:34:31 prosb Exp $
# $Log: fits_rheader.x,v $
# Revision 11.0  1997/11/06 16:34:31  prosb
# General Release 2.5
#
# Revision 9.0  1995/11/16 18:58:48  prosb
# General Release 2.4
#
#Revision 8.0  1994/06/27  15:20:28  prosb
#General Release 2.3.1
#
#Revision 7.0  93/12/27  18:39:58  prosb
#General Release 2.3
#
#Revision 6.0  93/05/24  16:24:41  prosb
#General Release 2.2
#
#Revision 5.0  92/10/29  21:36:51  prosb
#General Release 2.1
#
#Revision 1.2  92/10/16  10:01:20  mo
#New routine
#

#
# Module:       fits_rheader.x
# Project:      PROS -- ROSAT RSDC
# Purpose:      FITS IMAGE reader for MPE/MIDAS generated ROSAT images
# External:     < routines which can be called by applications>
# Local:        < routines which are NOT intended to be called by applications>
# Description:  This was adapted DIRECTLY from IRAF (dataio/rfits).  The
#		WCS parameter reading has been 'no-opted' and a parser
#		for the MIDAS/MPE HISTORY cards has been added, which also
#		generates a 'world' WCS.
# Copyright:    Property of Smithsonian Astrophysical Observatory
#               1989.  You may do anything you like with this
#               file except remove this copyright
# Modified:     {0} IRAF	initial version <when>    
#               {n} MC -- Hacked for MIDAS/FITS  -- 		Oct 92
...
#        call calloc(wcs, LEN_FTWCS, TY_STRUCT)
	...
	repeat {
        ...
        call mpe_imhead_const(fd_usr, card)
...
#pointer	wcs		# i/o: wcs structure  will add some more values

char	keyword[LEN_CARD]
char	value[20]
bool	key_found
int	mpe_instr
...
#           call calloc(CARDNA(info), SZ_CARDNA, TY_CHAR)
	    if( !key_found )
	       call rft_get_fits_string (card, keyword, SZ_FNAME)
#	    else
	    call strcpy(card[10], value, LEN_CARD)
                   #----------------------------------------------------
                   # Parse the MPE ASCII HISTORY record for keywords and
                   # values (including GTIs)
                   #----------------------------------------------------
            call mpe_imhead_parse(fd_usr, card, value, 
				key_found, keyword, mpe_instr) 
#  MPE/MIDAS files do NOT have a 'world' WCS in the FITS header cards. So
#	we will ignore the WCS included here and the mpe_imhead_parse
#	will build a 'world' WCS from the HISTORY records and predefined
#	constants describing the detector reference point and size
	} else if (strmatch (card, "CDELT1") != 0) {
#		    ndiscard = ndiscard + 1
	} else if (strmatch (card, "CDELT2") != 0) {
#		    ndiscard = ndiscard + 1
	} else if (strmatch (card, "CRPIX1") != 0) {
#		    ndiscard = ndiscard + 1
	} else if (strmatch (card, "CRPIX2") != 0) {
#		    ndiscard = ndiscard + 1
	} else if (strmatch (card, "CRVAL1") != 0) {
#		    ndiscard = ndiscard + 1
	} else if (strmatch (card, "CRVAL2") != 0) {
#		    ndiscard = ndiscard + 1
	} else if (strmatch (card, "CTYPE1") != 0) {
#		    ndiscard = ndiscard + 1
	} else if (strmatch (card, "CTYPE2") != 0) {
#		    ndiscard = ndiscard + 1
...
procedure mpe_get_fits_string (card, str, maxchar, istart, nchar)
...
begin
	# Check for opening quote
#	for (istart = COL_VALUE; istart <= LEN_CARD && card[istart] != '\'';
	for (istart = COL_VALUE-1; istart <= LEN_CARD && card[istart] != '\'';
	    istart = istart + 1)
	    ;
	istart = istart + 1
