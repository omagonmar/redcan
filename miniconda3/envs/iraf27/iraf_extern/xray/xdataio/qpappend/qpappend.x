#$Header: /home/pros/xray/xdataio/qpappend/RCS/qpappend.x,v 11.0 1997/11/06 16:37:38 prosb Exp $
#$Log: qpappend.x,v $
#Revision 11.0  1997/11/06 16:37:38  prosb
#General Release 2.5
#
...
#General Release 2.4
#
#Revision 8.1  1994/09/16  16:24:03  dvs
#Calls ev_strip with new argument.
#
#Revision 8.0  94/06/27  17:00:01  prosb
#General Release 2.3.1
#
#Revision 7.1  94/05/17  09:43:18  mo
#MC	5/12/94		Fixed bug when NO NTSI records present - forgot to
#			check existence of OUTPUT 'NTSI' header card
#
#Revision 7.0  93/12/27  18:46:26  prosb
#General Release 2.3
#
#Revision 6.4  93/12/22  17:11:24  mo
#no changes
#
#Revision 6.3  93/12/16  10:01:20  mo
#MC		12/15/93		Fix QPAPPEND to compare TSI record
#				definitions and eliminate TSI Records
#				if the defintion in any file doesn't match
#
#Revision 6.2  93/12/14  18:29:14  mo
#MC	12/13/93	Add special 'qpcreate' case of qpoe_list input
#			(not just qpoe)
#			Also fix length of strings and changes to qpx_addf
#
#Revision 6.1  93/07/02  15:11:25  mo
#MC	7/2/93		Correct declaraion of qp_accessf
#
#Revision 6.0  93/05/24  16:45:10  prosb
#General Release 2.2
#
#Revision 1.3  93/05/21  18:33:52  mo
#MC	5/21/93		Add support for General TSI records
#
#Revision 1.2  93/04/29  09:44:18  mo
#MC	4/27/93		Add special check for 0 aux records, apparently
#			the libraries do NOT handle this automatically
#
#Revision 1.1  93/04/27  11:52:24  mo
#Initial revision
#
...
extern	app_open(), app_get(), def_close()
extern	app_getparam(), app_hist(), app_finale()
...
#***                         2)
	...
	# 	name is changed
	call qpc_evload("qpoe_list", ".qp", app_open, app_get, def_close)
	# load getparam routine
	call qpc_parload(app_getparam)
        call qpc_finaleload(app_finale)
	# load history routine
	call qpc_histload(app_hist)
end

#
# APP_GETPARAM -- get region parameter for qpappend
#
procedure app_getparam(ifile, argv)
...
procedure app_finale(fd, qp, io, convert, qphead, display, argv)
...
#	call calloc(elist,xlen,TY_DOUBLE)
	  ...
	  if( Memi[VALID(argv)+i] == YES)
	  {
	    ...
	    call def_open(Memc[fname], fd, trecs, convert, temphead, display,
				argv) 
	    call app_tsiaux(fd[1],qphead,qp,i,display,argv)
...
#	duration = update_exp(Memc[filtkey],qp,ex,!gti,qphead)
	...
	call app_gtiaux(blist,elist,ngti,qp)
...
#
procedure app_hist(qp, qpname, file, qphead, display, argv)
...
#
procedure app_open(fname, fd, irecs, convert, qphead, display, argv)
...
#		buffer
#
procedure app_get(fd, evsize, convert, sbuf, get, got, qphead, display, argv)
...
#
procedure app_tsiaux(inqp,qphead,qp,fileno,display,argv)
...
procedure app_gtiaux(blist,elist,nlist,qp)
pointer	blist
pointer	elist
int	nlist
pointer	qp
