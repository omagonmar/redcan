...
procedure my_glabax (gp, title, xlabel, ylabel)
...
begin
	...
	xscaling = gstati (gp, G_XTRAN)
	yscaling = gstati (gp, G_YTRAN)
	call gseti (gp, G_XTRAN, GW_LINEAR)
	call gseti (gp, G_YTRAN, GW_LINEAR)
	call glb_setup (gp, axes, ntitlelines, xlabel, ylabel)
	call gseti (gp, G_XTRAN, xscaling)
	call gseti (gp, G_YTRAN, yscaling)
