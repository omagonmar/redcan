...
procedure ftr_fitr (imin, imout, ic, gt, title, xcenter, ycenter, radius,
...
begin
	...
	do i = yc - rmax, yc + rmax {
		...
		r = sqrt (real ((xc - j) ** 2 + (yc - i) ** 2))
		if (r <= rmax) {
		    Memr[x + npts] = r
		    Memr[y + npts] = Memr[imgl2r (imin, i) + j - 1]
		    npts = npts + 1
		}
	    }
	}
	...
	# Sort data
	call ftr_sortr (Memr[x], Memr[y], npts)
	...
	# Average data points
	call ftr_averager (Memr[x], Memr[y], npts, minwidth, minpts, ringavg)
	...
	if (interactive) {
	    ...
	    # Fit interactively
	    call icg_fitr (ic, gp, "cursor", gt, cv, Memr[x], Memr[y],
			    Memr[wts], npts)
	    ...
	    call ic_fitr (ic, cv, Memr[x], Memr[y], Memr[wts], npts,
			    YES, YES, YES, YES)
	...
	# Output data
	call ftr_outputr (imin, imout, cv, option, xc, yc, rmax) 
