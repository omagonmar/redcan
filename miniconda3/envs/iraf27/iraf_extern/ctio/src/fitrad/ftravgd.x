...
procedure ftr_averaged (x, y, npts, minwidth, minpts, ringavg)

double	x[npts]			# x data (in/out)
double	y[npts]			# y data (in/out)
...
begin
	...
	do n = 1, ninter {
	    ...
	    switch (ringavg) {
	    case RGV_MEAN:
	        call ftr_meand (x[ip], y[ip], nring, x[op], y[op])
	    case RGV_MEDIAN:
		call ftr_zmediand (x[ip], y[ip], nring, x[op], y[op])
	    case RGV_MODE:
		call ftr_moded (x[ip], y[ip], nring, x[op], y[op])
	    default:
...
procedure ftr_meand (x, y, npts, xavg, yavg)

double	x[npts]			# x data
double	y[npts]			# y data
int	npts			# number of points
double	xavg			# x point for average
double	yavg			# y avrage
...
begin
	xavg = (x[1] + x[npts]) / 2
	i = awvgd (y, npts, yavg, dummy, 0.0, 0.0)
end


# FTR_ZMEDIAN -- Compute the median of the y data

procedure ftr_zmediand (x, y, npts, xavg, yavg)
...
procedure ftr_moded (x, y, npts, xavg, yavg)
