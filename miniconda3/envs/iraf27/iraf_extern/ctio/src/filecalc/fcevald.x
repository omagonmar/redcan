...
double procedure fc_evald (code, fptr, warnings)
...
double	stack[STACK_DEPTH]	# evaluation stack
real	rval
double	dval

int	fc_fdata(), fc_cgeti()
real	fc_cgetr()
double	fc_vmind(), fc_vmaxd()
double	fc_vavgd(), fc_vmedd(), fc_vmoded(), fc_vsigmad()
double	fc_cgetd()
...
#		}
	    ...
	    case PEV_ACOS:
		if (abs (stack[sp]) - double (1) < EPSILOND)
		...
		stack[sp] = fc_vmind (stack[sp], Memi[code + ip])

	    case PEV_MAX:
		ip = ip + 1
		sp = sp - Memi[code + ip] + 1
		stack[sp] = fc_vmaxd (stack[sp], Memi[code + ip])


	    case PEV_AVG:
		ip = ip + 1
		sp = sp - Memi[code + ip] + 1
		stack[sp] = fc_vavgd (stack[sp], Memi[code + ip])

	    case PEV_MEDIAN:
		ip = ip + 1
		sp = sp - Memi[code + ip] + 1
		stack[sp] = fc_vmedd (stack[sp], Memi[code + ip])

	    case PEV_MODE:
		ip = ip + 1
		sp = sp - Memi[code + ip] + 1
		stack[sp] = fc_vmoded (stack[sp], Memi[code + ip], 1)

	    case PEV_SIGMA:
		ip = ip + 1
		sp = sp - Memi[code + ip] + 1
		stack[sp] = fc_vsigmad (stack[sp], Memi[code + ip])
