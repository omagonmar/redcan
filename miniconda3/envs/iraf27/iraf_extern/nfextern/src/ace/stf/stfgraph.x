...
procedure stf_graph (sf, interactive)

pointer	sf			#I Starfocus structure
bool	interactive		#I Interactive graphics?

real	wx, wy, x, y, r2, r2min, fa[8]
int	i, j, ix, iy, nx, ny, wcs, key, pkey, skey, del, redraw, clgcur()
...
begin
	...
	current = SF_BEST(sf)
	repeat {
	    # Ignore some keys under certain circumstances.
	    switch (key) {
	    case 'e', 'g', 'p', 'z':
		if (SF_LEVEL(sf) == 0.)
		    key = NULL
		...
		break
	    case '?':	# Help
		call gpagefile (gp, SF_HELP(sf), PROMPT)
		next
	    case ':':	# Colon commands
		...
		if ((SF_NS(sf) > 1 || SF_N(sf) > SF_NF(sf)) &&
		    (SF_NF(sf) > 1 || SF_N(sf) > SF_NS(sf))) {
		    if (SF_NS(sf) == 0 && (key == 'a' || key == 'b'))
		        key = 'f'
		    else if (key == 'b' && SF_NS(sf) == 1)
		        key = 'a'
		} else if (SF_NS(sf) > 1) {
		    if (key == 'b')
			key = 'a'
	    ...
	    case 'v':	# Undelete any sigma clipping
		j = 0
		do i = 1, SF_NSFD(sf) {
		    sfd = SF_SFD(sf,i)
		    if (SFD_STATUS(sfd) < 0) {
	    ...
	    switch (key) {
	    case 'r', 's', 'u', 'v', ':':	# Redraw last graph
		pkey = pkey
		nearest = current
		...
		default:
			...
			case 1, 2, 3, 4:
			    x = wx; y = wy; i = wcs
			default:
			    call printf ("y not supported in this graph")
			    key = pkey
			}
		    default:
			switch (wcs) {
		...
		case 'd':
			...
			sfs = SFD_SFS(nearest)
			do i = 1, SFS_NSFD(sfs)
			    SFD_STATUS(SFS_SFD(sfs,i)) = del
		    } else if (SF_N(sf) > 1)
			SFD_STATUS(nearest) = del
		    del = del + 1
		    call stf_fitfocus (sf)
		case 'x':
		    repeat {
			...
			case 'f':
				...
				sff = SFD_SFF(nearest)
				do i = 1, SFF_NSFD(sff)
				    if (SFD_STATUS(SFF_SFD(sff,i)) == 0)
					SFD_STATUS(SFF_SFD(sff,i)) = del
			    } else if (SF_N(sf) > 1)
				SFD_STATUS(nearest) = del
			    del = del + 1
			case 'i':
				...
				sfd = SFD_SFI(nearest)
				do i = 1, SFI_NSFD(sfd)
				    if (SFD_STATUS(SFI_SFD(sfd,i)) == 0)
					SFD_STATUS(SFI_SFD(sfd,i)) = del
			    } else if (SF_N(sf) > 1)
				SFD_STATUS(nearest) = del
			    del = del + 1
			case 'p':
		...
		case 'y':
			...
			wx, wy, wcs, key, Memc[cmd], SZ_LINE) != EOF) {
			if (wcs != i)
			    call printf ("Cursor is not in the same graph")
			else {
			    if (wx < x) {
			        r2 = x; x = wx; wx = r2
			    }
				...
				do i = 1, SF_NSFD(sf) {
				    ...
				    switch (wcs) {
				    case 1, 2:
					if (SFD_F(sfd) < x || SFD_F(sfd) > wx)
					    next
				    case 3, 4:
				        r2 = -2.5 * log10(SFD_M(sfd)/SF_M(sf))
					if (r2 < x || r2 > wx)
					    next
				    }
				    switch (wcs) {
				    case 1, 3:
					if (SFD_W(sfd) < y || SFD_W(sfd) > wy)
					    next
				    case 2, 4:
					if (SFD_E(sfd) < y || SFD_E(sfd) > wy)
					    next
				    }
				    SFD_STATUS(sfd) = del
				}
	    ...
	    # If there are no points undelete the last level.
	    if (SF_N(sf) == 0) {
		del = max (1, del-1)
		do i = 1, SF_NSFD(sf) {
		    sfd = SF_SFD(sf,i)
		...
		call stf_fitfocus (sf)
		call printf ("Deleting all data is not allowed")
	    }
...
#			"Ellipticity")
	    ...
	    if (pkey == 'a') {
		call sprintf (Memc[title], SZ_LINE, "Focus %.5g")
	    ...
	    clgcur ("graphcur", wx, wy, wcs, key, Memc[cmd], SZ_LINE)==EOF)
	...
	case SCALE:
		...
		do i = 1, SF_NSFD(sf) {
			...
			do j = 1, 19
			    SFD_FWHM(sfd,j) = SFD_FWHM(sfd,j) * rval
		    default:
			if (!IS_INDEFR(SFD_W(sfd)))
			    SFD_W(sfd) = SFD_W(sfd) * rval
		    }
		}
		do i = 1, SF_NSTARS(sf) {
		    sfd = SF_SFS(sf,i)
		    if (!IS_INDEFR(SFS_W(sfd)))
			SFS_W(sfd) = SFS_W(sfd) * rval
		}
		do i = 1, SF_NFOCUS(sf) {
		    sfd = SF_SFF(sf,i)
		    if (!IS_INDEFR(SFF_W(sfd)))
			SFF_W(sfd) = SFF_W(sfd) * rval
		}
		if (!IS_INDEFR(SF_W(sf)))
...
begin
	...
	do j = 1, SF_NFOCUS(sf) {
	    ...
	    if (xkey != 'f' && sff != SFD_SFF(current))
		next
	    if ((xkey == 'f' || xkey == 'm') && ykey == 'r' &&
	        SFF_NSFD(sff) > 0) {
		call malloc (w, SFF_NSFD(sff), TY_REAL)
		n = 0
	    }
	    do i = 1, SFF_NSFD(sff) {
		sfd = SFF_SFD(sff,i)
		if (SFD_STATUS(sfd) == 0) {
		    ...
		    switch (xkey) {
		    case 'f':
			x = SFD_F(sfd) + 0.05 * dx * (urand(seed)-0.5)
	    ...
	    if (xkey == 'f' && ykey == 'r' && n > 1) {
	        x = SFF_F(sff)
		#y = SFF_W(sff)
		y = asokr (Memr[w], n, (n + 1) / 2)
		call gmark (gp, x, y, GM_BOX, 5., 5.)
...
#	call gsview (gp, vx[2,1], vx[2,2], vy[1,1], vy[1,2])
	call gsview (gp, vx[2,1], vx[3,2], vy[1,1], vy[1,2])
	call gswind (gp, 0., 1., 0., 1.)
	call gfill (gp, fa, fa[5], 4, GF_SOLID)
...
#	call gsview (gp, vx[1,1], vx[1,2], vy[2,1], vy[2,2])
	call gsview (gp, vx[1,1], vx[1,2], vy[2,1], vy[3,2])
	call gswind (gp, 0., 1., 0., 1.)
	call gfill (gp, fa, fa[5], 4, GF_SOLID)
...
#	call gsview (gp, vx[2,1], vx[2,2], vy[2,1], vy[2,2])
	call gsview (gp, vx[2,1], vx[3,2], vy[2,1], vy[3,2])
	call gswind (gp, 0., 1., 0., 1.)
	call gfill (gp, fa, fa[5], 4, GF_SOLID)
...
begin
	...
	do i = 1, SF_NSTARS(sf) {
	    ...
	    if (SFS_N(sfs) == 0)
		next
	    x1 = min (x1, SFD_X(SFS_SFD(sfs,1)))
	    x2 = max (x2, SFD_X(SFS_SFD(sfs,1)))
	    y1 = min (y1, SFD_Y(SFS_SFD(sfs,1)))
	    y2 = max (y2, SFD_Y(SFS_SFD(sfs,1)))
	...
	if (abs (dvx) > 0.01 && abs (dvy) > 0.01) {
		...
		if (key == 1) {
			...
			z = sqrt (SFD_M(SFS_SFD(sfs,i)) / SF_M(sf))
		    z = max (0.005, 0.03 * z)
		    call gmark (gp, x, y, GM_MAG, z, z)
		}
