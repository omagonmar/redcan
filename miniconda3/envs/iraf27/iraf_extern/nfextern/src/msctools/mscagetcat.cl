...
begin
	...
	# There is a behavior in agetcat that needs to be fixed where a
	# temporary file is left behind.  Here we delete any file that
	# meets the pattern.  Obviously this has a small risk of a
	# problem if there is more than one agetcat running in the same
	# directory.  But this seems highly unlikely.
	delete ("query????*", verify-, >& "dev$null")
end
