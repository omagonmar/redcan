...
begin
	...
	while (imtgetim (input, Memc[mef], SZ_FNAME) != EOF) {
		...
		call cm_center (Memi[im], Memi[wcs], nims, Memi[imin],
		    Memd[xin], Memd[yin], Memd[xref], Memd[yref], Memi[sort], n,
		    nfit, cbox, maxshift, csig, cfrac, verbose, listcoords)
...
procedure cm_center (im, wcs, nims, imin, xin, yin, xref, yref, sort, n,
	nfit, cbox, maxshift, csig, cfrac, verbose, listcoords)
...
begin
	...
	do i = 1, nims {
		...
		if (listcoords == YES) {
		    call printf (
			"      %s %8.2f %8.2f %11.2H %11.1h %6.2f %6.2f %4.2f %4.2f")
			call pargstr (Memc[image])
			call pargd (x)
			call pargd (y)
			call pargd (xref[j])
