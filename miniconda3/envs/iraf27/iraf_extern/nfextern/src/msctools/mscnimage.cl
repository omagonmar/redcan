...
begin
	...
	int	nc, nl, cmin, cmax, lmin, lmax, nimage, nimages, nxblk, nyblk
	real	rval, xmin, xmax, ymin, ymax, crpix1, crpix2
	string	extname, str
	...
	while (fscan (fd_in, in, out) != EOF) {
	    ...
	    # Create WCS template.  This is created only once per MEF file.
	    if (!imaccess(wcsref)) {
		ref = reference
		...
		delete (coord, verify-)
		wcsctran (db, coord, wcsref, inwcs="world", outwcs="logical",
		...
		if (pixmask) {
		    ...
		    hedit (outsec, "BPM", del+, add-, show-, verify-, update+)
		...
		if (pixmask) {
			...
			obstype="", detector="", group="", delete=no,
			...
			statsec="", lthreshold=INDEF, hthreshold=.99,
