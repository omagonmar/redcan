...
errchk	fcopy, open, immap, imdelete, imrename, ctcopy, ctread, ctimmap, ctalk
...
#	    call error (2, "xtalkcor: no crosstalk file specified")
	...
	while (imtgetim (inlist, Memc[infile], SZ_FNAME) != EOF) {
		...
		do j = nstps, 1, -1 {
			...
			i = Memi[sym] - 1
			iferr (ptr = ctimmap (Memc[infile], instrume,
		...
		# Copy the images without a correction if splitting.
		    ...
		    call ctsplit (Memc[infile], instrume, Memc[outfile],
		        Memc[tempfile], bufsize, Memc[xtfile1], outlist)
	    } then {
		ierr = errget (Memc[err], SZ_LINE)
		if (ierr > 1) {
		    do i = 1, nimages {
...
int	i, ampid, x1, x2, x3, y1, y2, y3, strlen()
...
begin
	...
	case DECAM:
	    ...
	    i = strlen (extname)
	    call strcpy (extname, Memc[str], i-1)
	    ampid = extname[i]
	    switch (ampid) {
	    case 'A':
		call sprintf (output, szoutput, "%s[%s][1081:2154,51:4146]")
		call pargstr (input)
		call pargstr (Memc[str])
		im = immap (output, READ_ONLY, 0)
		call imgstr (im, "AMPSECA", Memc[str], SZ_FNAME)
	    case 'B':
		call sprintf (output, szoutput, "%s[%s][7:1080,51:4146]")
		call pargstr (input)
		call pargstr (Memc[str])
		im = immap (output, READ_ONLY, 0)
		call imgstr (im, "AMPSECB", Memc[str], SZ_FNAME)
	    }
	...
	iferr {
		...
		case DECAM:
			...
			case 'A':
				...
				call sprintf (Memc[inname], SZ_FNAME,
				    "%s[%s][1081:2154,51:4146]")
				call pargstr (infile)
				call pargstr (Memc[extname])
				tmp = immap (Memc[inname], READ_ONLY, 0)
				Memc[inname] = EOS
			    }
			case 'B':
...
procedure ctsplit (infile, instrume, outfile, tempfile, bufsize, xtfile, outlist)
...
begin
	...
	case DECAM:
	    ...
	    switch (ampid) {
	    case 'A':
		call imgstr (in, "DETSECA", Memc[str], SZ_FNAME)
		call imastr (out, "DETSEC", Memc[str])
		call imgstr (in, "CCDSECA", Memc[str], SZ_FNAME)
		call imastr (out, "CCDSEC", Memc[str])
		call imastr (out, "DATASEC", "[1:1024,*]")
		call imastr (out, "TRIMSEC", "[1:1024,*]")
		call imastr (out, "BIASSEC", "[1025:1074,*]")
		rval = imgetr (in, "GAINA")
		call imaddr (out, "GAIN", rval)
		rval = imgetr (in, "RDNOISEA")
		call imaddr (out, "RDNOISE", rval)
		rval = imgetr (in, "SATURATA")
	    ...
	    case 'B':
		call imgstr (in, "DETSECB", Memc[str], SZ_FNAME)
		call imastr (out, "DETSEC", Memc[str])
		call imgstr (in, "CCDSECB", Memc[str], SZ_FNAME)
		call imastr (out, "CCDSEC", Memc[str])
		call imastr (out, "DATASEC", "[51:1074,*]")
		call imastr (out, "TRIMSEC", "[51:1074,*]")
		call imastr (out, "BIASSEC", "[1:50,*]")
		rval = imgetr (in, "GAINB")
		call imaddr (out, "GAIN", rval)
		rval = imgetr (in, "RDNOISEB")
		call imaddr (out, "RDNOISE", rval)
		rval = imgetr (in, "SATURATB")
		call imaddr (out, "SATURATE", rval)
		rval = imgetr (in, "CRPIX1")
		rval = rval - 6 
		call imaddr (out, "CRPIX1", rval)
