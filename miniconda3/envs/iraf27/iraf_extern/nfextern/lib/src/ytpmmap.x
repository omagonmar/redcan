...
int	i, j, k, nc, nl, ncpm, nlpm, nxmsi, nymsi
int	c_im, l_im, c_ref, l_ref, c1_ref, c2_ref, l1_ref, l2_ref
int	xmin, xmax, ymin, ymax
double	pix_im[2], pix_ref[2], pix_tmp[2]
real	x, y, step, d[2], der[2,2]
long	v[2]
pointer	sp, bits, rl
...
begin
	...
	do j = 0, nymsi-1 {
		...
		if (x > 0.5 && x < nc+0.5 && y > 0.5 && y < nl+0.5) {
		    xmin = min (xmin, nint(pix_im[1]))
		    xmax = max (xmax, nint(pix_im[1]))
		    ymin = min (ymin, nint(pix_im[2]))
		    ymax = max (ymax, nint(pix_im[2]))
		}
		Memr[xvec+k] = x
		Memr[yvec+k] = y
		k = k + 1
	...
	x = (x - 1) * step + 1; y = (y - 1) * step + 1
	call msider (xmsi, x, y, der, 2, 2, 2)
	d[1] = max (abs(der[2,1]), abs(der[1,2])) * step
	call msider (ymsi, x, y, der, 2, 2, 2)
	d[2] = max (abs(der[2,1]), abs(der[1,2])) * step
