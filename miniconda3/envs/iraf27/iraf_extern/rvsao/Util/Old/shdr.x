...
# SPHD_OPEN    -- Open the SHDR spectrum header structure.
# SPHD_GTYPE   -- Get the selected spectrum type.
# SPHD_CLOSE   -- Close and free the SHDR structure.
# SPHD_COPY    -- Make a copy of an SHDR structure.
# SPHD_SYSTEM  -- Set or change the WCS system.
# SPHD_UNITS   -- Set or change user units.
...
# SPHD_OPEN -- Open SHDR spectrum header structure.
#
...
procedure sphd_open (im, smw, index1, index2, ap, mode, sh)
...
begin
	...
	if (newim) {
	    ...
	    call sphd_gi (im, "OFLAG", OBJECT, OFLAG(sh))
	    call sphd_gr (im, "EXPOSURE", INDEF, IT(sh))
	    call sphd_gr (im, "ITIME", IT(sh), IT(sh))
	    call sphd_gr (im, "EXPTIME", IT(sh), IT(sh))
	    call sphd_gr (im, "RA", INDEF, RA(sh))
	    call sphd_gr (im, "DEC", INDEF, DEC(sh))
	    call sphd_gr (im, "UT", INDEF, UT(sh))
	    call sphd_gr (im, "ST", INDEF, ST(sh))
	    call sphd_gr (im, "HA", INDEF, HA(sh))
	    call sphd_gr (im, "AIRMASS", INDEF, AM(sh))
...
data_	# Set the coordinate and data arrays if desired otherwise free them.
	...
	W0(sh) = sphd_lw (sh, double(1))
	W1(sh) = sphd_lw (sh, double(np))
	...
	do i = 1, np
	    Memr[SX(sh)+i-1] = sphd_lw (sh, double(i))
...
procedure sphd_copy (sh1, sh2, wcs)
...
procedure sphd_system (sh, system)
...
begin
	smw = MW(sh)
	if (smw == NULL)
	    call error (1, "sphd_system: MWCS not defined")
...
# SPHD_UNITS -- Set or change the WCS system.
# This changes W0, W1, WP, and SX.

procedure sphd_units (sh, units)
...
# SPHD_LW -- Logical to world coordinate transformation.
# The transformation pointer is generally NULL only after SPHD_LINEAR
...
double	x, w, xmin, xmax, sphd_lw(), sphd_wl()
...
procedure sphd_linear (sh, w0, w1, sn, dc)
...
int	i, j, type, ia, ib, n, clgwrd()
real	a, b, sum, asieval(), asigrl()
double	x, w, w0l, wp, xmin, xmax, sphd_wl()
pointer	unsave, asi, spec
bool	fp_equalr()
...
procedure sphd_extract (sh, w1, w2, rebin)
...
pointer	buf
bool	fp_equald()
double	sphd_wl(), sphd_lw()
errchk	sphd_linear, sphd_lw, sphd_wl

begin
	l1 = sphd_wl (sh, double (w1))
	l2 = sphd_wl (sh, double (w2))
	...
	if (rebin) {
	    ...
	    l2 = sphd_lw (sh, l2)
	    if (DC(sh) == DCFUNC)
		call sphd_linear (sh, real (l1), real (l2), n, DCLINEAR)
	    else
		call sphd_linear (sh, real (l1), real (l2), n, DC(sh))
	} else {
	    if (i1 == 1 && i2 == SN(sh))
		return
...
# SPHD_GD -- Load a double value from the header
...
# Aug  7 1996	Add code to free smw structure in SPHD_CLOSE
