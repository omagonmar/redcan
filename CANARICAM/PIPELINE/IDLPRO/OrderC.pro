...
for j=0,n_elements(aux) -1 do begin & $ 
	if  strtrim(grating(aux(j)),0) eq "Mirror" then begin & $
		TYPE_IS(aux(j)) = "IMAGE"  & $
	endif else begin & $
...
for j=0,n_elements(aux) -1 do begin & $ 
;	if obsclass(aux(j)) eq "partnerCal" or  obsclass(aux(j)) eq "progCal"  or  obsclass(aux(j)) eq "dayCal" then begin & $
	if strtrim(obsclass(aux(j)),2) eq "CALIB" then begin & $
		TYPE_TSA(aux(j))= "STANDARD" & $
	endif & $
	if strtrim(obsclass(aux(j)),2) eq "SCIENCE" then begin & $
		TYPE_TSA(aux(j))= "TARGET" & $
	endif & $
	if strtrim(obsclass(aux(j)),2) eq "ACQ" then begin & $
;	if obsclass(aux(j)) eq "acq" or  obsclass(aux(j)) eq "acqCal" then begin & $
...
printf,1 , "?===TARGET/STANDARD/ACQUISITION'
close,1
