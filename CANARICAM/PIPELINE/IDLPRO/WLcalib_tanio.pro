...
for i = 0 , n_elements(filename) -1 do begin
	...
	on = spectrum(*,*,0)
    	off = spectrum(*,*,1)
    	onoff= spectrum(*,*,2)
        ; we add the on- and off-source image thinking that,
        ; in the future, we might want to chop on-source 
    	;for l = 0,n_elements(on(0,*))-1 do for m = 0, n_elements(off(*,0))-1 do off(m,l)=total([on(m,l),off(m,l)]) ---> MODIFIED TO:
        off=on+off
	;
	; Colapsing all
	;
   	;offsmooth = off(*,0)
 	;for m = 0 , n_elements(off(*,0)) -1 do offsmooth(m) = total(off(m,*)) ---> MODIFIED TO:
...
;	close,3
	...
	;
        ...
        for l = 5 , n_elements(off(*,0)) -6 do begin 
           ;if offsmooth(l-1)  lt offsmooth(l) and offsmooth(l-2) lt offsmooth(l) and offsmooth(l-3)  lt offsmooth(l) and offsmooth(l-4) lt offsmooth(l) and offsmooth(l-5) lt offsmooth(l) and offsmooth(l+1) lt offsmooth(l) and offsmooth(l+2) lt offsmooth(l) and offsmooth(l+3) lt offsmooth(l) and offsmooth(l+4) lt offsmooth(l) and offsmooth(l+5) lt offsmooth(l) then begin ---> modified to:
...
; 	   oplot, xxx, zpos_vec, linestyle =2 
 ...
 ;       ;
 ;       ; Space Calibration per column
 ;       ;
 ;       for k = 0, n_elements(line_sel) -1 do begin
 ;          zvec= wavecalim(k,auxi)
 ;          yvec= auxi
 ;         av =  poly_fit(yvec,zvec,3, yfit = yyfit)
 ;          xxx = indgen(n_elements(off(0,*)))
...
;        endfor
        ...
        collwavecalim=wmean(wavecalim,dim=2)
        ncollwavecalim=n_elements(collwavecalim)
        params=poly_fit(indgen(ncollwavecalim),collwavecalim,1)
        collfit=indgen(ncollwavecalim)*params[1]+params[0]       
	plot,indgen(ncollwavecalim),collwavecalim,color=0,psym=4,XTITLE='Ncol',YTITLE='Wavelength',yrange=[-400,400];yrange=[7.5e4,1.6e5],ystyle=1,/nodata

        modhdr=hdr
        FXADDPAR, modhdr, 'NAXIS3', 1 ,'Number of positions along axis 3'
        FXADDPAR, modhdr, 'LTV1', 1.-(params[0]/params[1]), ' '
        FXADDPAR, modhdr, 'LTV2', 0., ' '
        FXADDPAR, modhdr, 'LTM1_1', 1./params[1], ' '
        FXADDPAR, modhdr, 'LTM2_2', 1., ' '
        writefits,'PRODUCTS/SWC_'+prefix+'_'+filename(i),spectrum[*,*,2],modhdr
