pro extract_spec,objname,REFPOSAPER=refposaper,REFWIDAPER=refwidaper,APERW=aperw,FIXAPER=fixaper,NAME=name,SAVE=save,ASK=ask,FIXROW=fixrow,SILENT=silent,STEPS=steps,GALAXY=galaxy,INPUTS=inputs,NOEXTEN=noexten,ROOTDIR=rootdir,DEBUG=debug
...
;rootdir='/home/tanio/UFLdata/spec/'
;rootdir='/astro/homes/santos/data/'+galaxy+'_spec/'
;rootdir='/home/tanio/data/trecs/sample/spectroscopy/'+galaxy+'/postredux/'
;rootdir='/data1/trecs_spec/working/'+galaxy+'/postredux/'
IF keyword_set(rootdir) EQ 0 THEN rootdir='/data1/trecs_spec/working/'
dir=rootdir+galaxy+'/postredux/'
IF n_params() EQ 0 THEN message,'Tienes que dar el nombre del objeto'
IF keyword_set(name) EQ 0 THEN name=objname+'_1dext'
...
; OBJECT
head=headfits(dir+objname+'.fits')
ima=readfits(dir+objname+'.fits',headext1,/NOSCALE,EXTEN=exten)
;ima=ima*1000.
imasize=size(ima)

IF keyword_set(noexten) EQ 0 THEN BEGIN
...
IF strmid(name,0,2) NE 'HD' THEN BEGIN
    sivlamb=where(lambs GT 105100.*1.015) & sivlamb=sivlamb[0]
    pahlamb1=where(lambs GT 112500.*1.015) & pahlamb1=pahlamb1[0]
    pahlamb2=where(lambs GT 86000.*1.015) & pahlamb2=pahlamb2[0]
    siabslamb=where(lambs GT 96000.*1.015) & siabslamb=siabslamb[0]
    lambweight[pahlamb2-10:pahlamb2+10]=0. ;UTILIZAR??????????????
    lambweight[sivlamb-10:sivlamb+10]=0. ;UTILIZAR??????????????
    lambweight[pahlamb1-10:pahlamb1+10]=0. ;UTILIZAR??????????????
...
IF keyword_set(fixrow) EQ 0 THEN BEGIN
    ...
    refrow=term[1]
ENDIF ELSE BEGIN
; SI HEMOS ESCOGIDO FIJAR LA FILA DE REFERENCIA (Y HEMOS PREGUNTADO),
; LA FILA DE REFERENCIA SERA EN LA QUE HEMOS 'CLICKADO'
    IF keyword_set(ask) THEN BEGIN
        fitgauss=fltarr(imasize[2])
        refrow=gauss1diniposfit;=pos[1,0]
    ENDIF ELSE BEGIN
; SI LA FILA ES FIJA CALCULAMOS LA FILA DE REFERENCIA CON EL OFFSET
; RESPECTO AL NUCLEO
