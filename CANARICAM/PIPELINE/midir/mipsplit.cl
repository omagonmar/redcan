...
begin
    ...
    if (substr(l_inputimages,1,1) == "@") {
        ...
        if (!access(l_temp) && !access(l_rawpath//l_temp)) {
            glogprint( l_logfile, "mipsplit", "status", type="error", errno=101,
    ...
    while (i <= nimages) {
        ...
        if (instrument == "michelle") {
            ...
            glogprint (l_logfile, "mipsplit", "status", type="error",
                errno=138, str="Error: for image "//in[i]//" the instrument used is not MICHELLE",verbose+)
            status = status+1
            goto nextimage
        }
        ...
        if (j < 1) {
            glogprint (l_logfile, "mipsplit", "status", type="error", errno=123,
                ...
                for (m=1; m <= nnods; m=m+1) {
                    for (j=8*(m-1)+1; j <= 8*m ; j=j+1) {
                        ...
                        if ((jwp > 0) && (jwp < 5)) {
                            ...
                            imcopy (in[i]//"["//j//"][*,*,3]", filename,ver-, >& "dev$null")
                            imcopy (in[i]//"["//j//"][*,*,3]", tmpwork,ver-, >& "dev$null")
                            ...
                            if (l_fl_stair) {
                              for (m=1; m <= 301; m=m+20) {
                                ...
                                imarith(tmpwork//"["//str(m)//":"//str(m+19)//",1:240]","-",chlevel,
                                  tmpwork1)
                                imcopy(tmpwork1,tmpwork//"["//str(m)//":"//str(m+19)//",1:240]",ver-, >& "dev$null")
                                imdelete(tmpwork1,ver-, >& "dev$null")
                              }
                              imdelete(filename,ver-, >& "dev$null")
                              imcopy (tmpwork,filename,ver-, >& "dev$null")
                            }
                            ...
                            if (l_fl_register) {
                              if (nwp[jwp] == 1) {
                                ...
                                imcopy(tmpwork,tmpwork1,ver-, >& "dev$null")
                              }
                              else {
                                if (jwp == 1) {
                                  xregister(tmpwork,tmpregister1,regions=l_regions,shifts=tmpshifts,output=tmpwork1,
                                ...
                                if (jwp == 2) {
                                  xregister(tmpwork,tmpregister2,regions=l_regions,shifts=tmpshifts,output=tmpwork1,
                                    background="none",loreject=INDEF,hireject=INDEF,apodize=0.,filter="none",
                                    append+,records="",correlation="discrete",xwindow=11,ywindow=11,xcbox=11,ycbox=11,
                                    function="centroid",interp_type="poly5",interact-,xlag=0,ylag=0,dxlag=0,dylag=0)
                                }
                                if (jwp == 3) {
                                  xregister(tmpwork,tmpregister3,regions=l_regions,shifts=tmpshifts,output=tmpwork1,
                                    background="none",loreject=INDEF,hireject=INDEF,apodize=0.,filter="none",
                                    append+,records="",correlation="discrete",xwindow=11,ywindow=11,xcbox=11,ycbox=11,
                                    function="centroid",interp_type="poly5",interact-,xlag=0,ylag=0,dxlag=0,dylag=0)
                                }
                                if (jwp == 4) {
                                  xregister(tmpwork,tmpregister4,regions=l_regions,shifts=tmpshifts,output=tmpwork1,
                                    background="none",loreject=INDEF,hireject=INDEF,apodize=0.,filter="none",
                                    append+,records="",correlation="discrete",xwindow=11,ywindow=11,xcbox=11,ycbox=11,
                                    function="centroid",interp_type="poly5",interact-,xlag=0,ylag=0,dxlag=0,dylag=0)
                                }
                              ...
                              imdelete(filename,ver-, >& "dev$null")
                              imcopy (tmpwork1,filename,ver-, >& "dev$null")
                            }

                            imdelete(tmpwork,ver-, >& "dev$null")
                            imdelete(tmpwork1,ver-, >& "dev$null")

                            glogprint (l_logfile, "mipsplit", "task", type="string",
                                str="  "//in[i]//"["//j//"][*,*,3] --> "//filename, verbose=l_verbose)
                            if (jwp == 1) {
                                hedit (filename, "WPLATE", 0.0, add+, addonly+,
                                    delete-, verify-, show-, update+)
                                hedit (filename, "POLANGLE", 0.0, add+,
                                    addonly+, delete-, verify-, show-, update+)
                            }
                            if (jwp == 2) {
                                hedit (filename, "WPLATE", 22.5, add+,
                                    addonly+, delete-, verify-, show-, update+)
                                hedit (filename, "POLANGLE", 45.0, add+,
                                    addonly+, delete-, verify-, show-, update+)
                            }
                            if (jwp == 3) {
                                hedit (filename, "WPLATE", 45.0, add+,
                                    addonly+, delete-, verify-, show-, update+)
                                hedit (filename, "POLANGLE", 90.0, add+,
                                    addonly+, delete-, verify-, show-, update+)
                            }
                            if (jwp == 4) {
                                hedit (filename, "WPLATE", 67.5, add+,
                                    addonly+, delete-, verify-, show-, update+)
                                hedit (filename, "POLANGLE", 135.0, add+,
                                    addonly+, delete-, verify-, show-, update+)
                            }

                            # add static parameters (to be determined on sky...)

                            hedit (filename, "ANGROT", 0.0, add+, addonly+, delete-,
                              verify-, show-, update+)
                            hedit (filename, "T", 1.0, add+, addonly+, delete-,
                              verify-, show-, update+)
                            hedit (filename, "EPS", 1.0, add+, addonly+, delete-,
                              verify-, show-, update+)
                            
                            # Copy WCS information to the stacked image from the primary header
                            imgets(header,"CTYPE1")
                            if (imgets.value != "") {
                              gemhedit(filename,"WCSAXES",2,"Number of WCS axes in the image")
                              imgets(header,"CTYPE1")
                              gemhedit(filename,"CTYPE1",imgets.value,"R.A. in tangent plane projection")
                            }
                            imgets(header,"CRPIX1")
                            if (imgets.value != "") gemhedit(filename,"CRPIX1",imgets.value,"Ref pix of axis 1")
                            imgets(header,"CRVAL1")
                            if (imgets.value != "") gemhedit(filename,"CRVAL1",imgets.value,"RA at Ref pix in decimal degrees")
                            imgets(header,"CTYPE2")
                            if (imgets.value != "") gemhedit(filename,"CTYPE2",imgets.value,"DEC. in tangent plane projection")
                            imgets(header,"CRPIX2")
                            if (imgets.value != "") gemhedit(filename,"CRPIX2",imgets.value,"Ref pix of axis 2")
                            imgets(header,"CRVAL2")
                            if (imgets.value != "") gemhedit(filename,"CRVAL2",imgets.value,"DEC at Ref pix in decimal degrees")
                            imgets(header,"CD1_1")
                            if (imgets.value != "") gemhedit(filename,"CD1_1",imgets.value,"WCS matrix element 1 1")
                            imgets(header,"CD1_2")
                            if (imgets.value != "") gemhedit(filename,"CD1_2",imgets.value,"WCS matrix element 1 2")
                            imgets(header,"CD2_1")
                            if (imgets.value != "") gemhedit(filename,"CD2_1",imgets.value,"WCS matrix element 2 1")
                            imgets(header,"CD2_2")
                            if (imgets.value != "") gemhedit(filename,"CD2_2",imgets.value,"WCS matrix element 2 2")
                            imgets(header,"RADECSYS")
                            if (imgets.value != "") gemhedit(filename,"RADECSYS",imgets.value,"R.A./DEC. coordinate system reference")

                            gemdate ()
                            nhedit (filename, "GEM-TLM", gemdate.outdate,
                             "UT Last modification with GEMINI", comfile="NULL", after="",
                             before="", update+, add+, addonly-, delete-, 
                             verify-, show-)
                            nhedit (filename, "MIPSPLIT", gemdate.outdate,
                                "UT Time stamp for MISPLIT", comfile="NULL",
