...
begin
    ...
    char    combimagelist
    char    tmpshift, tmpfinal, tmpshift1, tmpmask, tmplog
    ...
    for (i=1; i <= nexts; i=i+7) {
        if (l_fl_register) {
            if (i == 1) {
                ...
                #imcopy (l_inimage//"[1][*,*]", refimage, ver-, >& "dev$null")
				imcopy (l_inimage//"[1]", refimage, ver-, >& "dev$null")
                print (refimage, > imagelist)
            }
            tmpimage = mktemp ("tmpimage")
            imcopy (l_inimage//"["//i//"]", tmpwork[1], ver-, >& "dev$null")
            xregister (tmpwork[1], refimage, regions=l_regions, shifts=tmpshift,
                output=tmpimage, background="none", loreject=INDEF,
                hireject=INDEF, apodize=0., filter="none", append+, records="",
                correlation="discrete", xwindow=11, ywindow=11,
                function="centroid", interp_type="poly5", interact-, xlag=0,
                ylag=0, dxlag=0, dylag=0, xcbox=5, ycbox=5, >& tmplog)
            ...
            imdelete (tmpwork[1], yes, ver-, >& "dev$null")
        } else {
            tmpimage = mktemp("tmpimage")
            imcopy (l_inimage//"["//i//"]", tmpimage, ver-, >& "dev$null")
            print (tmpimage, >> imagelist)      
        }
    }
    ...
    imcombine ("@"//imagelist, tmpwork[1], combine="average", headers="",
        ...
        reject="none", project-, outtype="double", outlimits="", offsets="none",
    ...
    for (j=2; j <= 3; j=j+1) {
        ...
        delete (newimagelist, ver-, >& "dev$null")
    ...
    tmp1 = mktemp ("tmpwork")
    if (l_fl_stair) {
        # In the following, check for midpt => INDEF, which happens when all the
        # values are the same...although that should mean midpt is the same as 
        # any one of the values.  This can happen if the entire region is 
        ...
        for (m=1; m <= 301; m=m+20) {
            ...
            if (stridx(midval,"INDEF") == 0) {
                ...
                imarith (tmpwork[2]//"["//str(m)//":"//str(m+19)//",1:240]",
                    "-", chlevel, tmp1)
                imcopy (tmp1,
                    tmpwork[2]//"["//str(m)//":"//str(m+19)//",1:240]", ver-,
                    >& "dev$null")
                imdelete (tmp1, ver-, >& "dev$null")
            }
        }
        for (m=1; m <= 301; m=m+20) {
            ...
            if (stridx(midval,"INDEF") == 0) {
                ...
                imarith (tmpwork[3]//"["//str(m)//":"//str(m+19)//",1:240]",
                    "-", chlevel, tmp1)
                imcopy (tmp1, 
                    tmpwork[3]//"["//str(m)//":"//str(m+19)//",1:240]", ver-,
                    >& "dev$null")
                imdelete (tmp1, ver-, >& "dev$null")
            }
        }
    ...
    if (l_fl_mask) {
        if (l_noise <= 0.) {
            ...
            # S/N required is 282 for 0.5% accuracy in the final image, goes as the 
            # inverse square of the accuracy required.
        ...
        imcopy (tmpwork[1], tmpmask, ver-, >& "dev$null")
        ...
        imrename (tmp1, tmpmask, ver-, >& "dev$null")
        nmisc.fixpix (tmpwork[2], tmpmask, creplace="1-10", linterp="",
            cinterp="", constant=0.)
        nmisc.fixpix (tmpwork[3], tmpmask, creplace="1-10", linterp="",
            cinterp="", constant=0.)
        imdelete (tmpmask, ver-, >& "dev$null")
    }
    ...
    for (i=1; i<=7; i+=1) {
        nhedit (l_outimage//".fits["//str(i)//"]", "EXTNAME", "SCI",
            "Extension name", comfile="NULL", after="", before="", update+,
            add+, addonly-, delete-, verify-, show-)
        nhedit (l_outimage//".fits["//str(i)//"]", "EXTVER", i,
            "Extension version", comfile="NULL", after="", before="", update+,
            add+, addonly-, delete-, verify-, show-)
    }
    
    if (l_fl_variance) {
        imfunction (sigin[1], sigin[1], "square", ver-, >& "dev$null")
        fxinsert (sigin[1], l_outimage//".fits[7]", "", >& "dev$null")
        imfunction (sigin[2], sigin[2], "square", ver-, >& "dev$null")
        fxinsert (sigin[2], l_outimage//".fits[8]", "", >& "dev$null")
        imfunction (sigin[3], sigin[3], "square", ver-, >& "dev$null")
        fxinsert (sigin[3], l_outimage//".fits[9]", "", >& "dev$null")
        for (i=1; i<=3; i+=1) {
            nhedit (l_outimage//".fits["//str(i+7)//"]", "EXTNAME", "VAR",
                "Extension name", comfile="NULL", after="", before="", update+,
                add+, addonly-, delete-, verify-, show-)
            nhedit (l_outimage//".fits["//str(i+7)//"]", "EXTVER", i,
                "Extension version", comfile="NULL", after="", before="", update+,
                add+, addonly-, delete-, verify-, show-)
        }
    ...
    gemdate()
    nhedit (l_outimage//".fits[0]", "MIPSSTK", gemdate.outdate,
        "UT Time stamp for MIPSSTK", comfile="NULL", after="", before="",
        update+, add+, addonly-, delete-, verify-, show-)
    nhedit (l_outimage//".fits[0]", "GEM-TLM", gemdate.outdate,
        "UT Last modification with GEMINI", comfile="NULL", after="", before="",
        update+, add+, addonly-, delete-, verify-, show-)

    imdelete (tmpwork[1], ver-, >& "dev$null")
    imdelete (tmpwork[2], ver-, >& "dev$null")
    imdelete (tmpwork[3], ver-, >& "dev$null")
    imdelete (tmp1, ver-, >& "dev$null")
    imdelete (tmp2, ver-, >& "dev$null")
    imdelete (tmp3, ver-, >& "dev$null")
    imdelete (tmp4, ver-, >& "dev$null")

    delete ("tmpshift*", ver-, >& "dev$null")
    delete ("tmpimage*", ver-, >& "dev$null")
    delete ("tmpsigma*", ver-, >& "dev$null")
    delete ("tmprefimage*", ver-, >& "dev$null")
    delete ("tmpimagelist*", ver-, >& "dev$null")
