    ...
    while (i <= nimages) {
        ...
        if (imgets.value == "0") {
            ...
            if (imgets.value == "0") {
                ...
                nhedit (in[i]//"[0]", "ENDTIME", str(start), 
                    "UT at observation end", comfile="NULL", after="",
                    before="", update=l_update, add-, addonly+, delete-, 
                    verify-, show+, >& tmplog)
                start = n * 45.
                nhedit (in[i]//"[0]", "OBSTIME", str(start), 
                    "[s] Duration of observation", comfile="NULL", after="",
                    before="", update=l_update, add-, addonly+, delete-,
                    verify-, show+, >>& tmplog)
                ...
                nhedit (in[i]//"[0]", "OBSEND", newtime, "obsEnd", 
                    comfile="NULL", after="", before="", update=l_update, add-, 
                    addonly+, delete-, verify-, show+, >& tmplog)
                nhedit (in[i]//"[0]", "OBSCLASS", "unknown", "Observe class",
                    comfile="NULL", after="", before="", update=l_update, add-,
                    addonly+, delete-, verify-, show+, >>& tmplog)
                glogprint (l_logfile, "mcheckheader", "task", type="file",
                    str=tmplog, verbose=l_verbose)
