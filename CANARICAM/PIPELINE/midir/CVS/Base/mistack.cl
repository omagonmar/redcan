    ...
    while (i <= nimages) {
        ...
        if (modeflag == 1) {
            ...
            if (j != 1) {
                ...
                # Time stamps
                gemdate ()
                nhedit (out[i]//"[0]", "MISTACK", gemdate.outdate,
                    "UT Time stamp for MISTACK", comfile="NULL", after="",
                ...
                nhedit (out[i]//"[0]", "GEM-TLM", gemdate.outdate,
                    ...
                    before="", update+, add+, addonly-, delete-, verify-, show-)
                hedit(out[i]//"[1]","AXISLAB4","",add-,addonly-,delete+,verify-,show-,update+)
                hedit(out[i]//"[1]","AXISLAB3","",add-,addonly-,delete+,verify-,show-,update+)
            }
