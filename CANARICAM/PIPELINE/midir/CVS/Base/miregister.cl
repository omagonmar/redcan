...
begin
    ...
    char    l_inputimages,l_outputimages,l_filename,l_prefix,l_logfile,l_rawpath
    char    l_combine
    ...
    while (i <= nimages) {
        ...
        if (modeflag == 1) {
            ...
            for (j=1; j < l_extensions; j=j+2) {
                ...
                if (imgets.value != "0") {
                    ...
                    if (imgets.value != "0")
                        ...
                        xregister(in[i]//"["//j//"]"//"[*,*,3]",tmprefimage,
                            l_regions,tmpregister,output=tmpshift1,interac-,verbose-)
                        delete(tmpregister,ver-, >& "dev$null")
                        xregister(in[i]//"["//j+1//"]"//"[*,*,3]",tmprefimage,
                            l_regions,tmpregister,output=tmpshift2,interac-,verbose-)
                        delete(tmpregister,ver-, >& "dev$null")
                        imarith(tmpshift1,"+",tmpshift2,tmpfile2,ver-)
                        ...
                        imdelete(tmpshift1//","//tmpshift2,ver-,>& "dev$null")
                    }
                }
            }
            ...
            # Time stamps
            gemdate ()
            nhedit (out[i]//"[0]", "MIREGIST", gemdate.outdate,
                "UT Time stamp for MIREGISTER", comfile="NULL", after="",
            ...
            hedit(out[i]//"[1]","AXISLAB4","",add-,addonly-,delete+,verify-,
                show-,update+)
            hedit(out[i]//"[1]","AXISLAB3","",add-,addonly-,delete+,verify-,
                show-,update+)
        }
        ...
        if (modeflag == 2) {
            ...
            imcopy(in[i]//"[1][*,*,3].fits",tmpfile1,ver-)
            print(tmpfile1, >> tmpon)
            ...
            for (j=1; j < l_extensions; j=j+2) {
                ...
                if (imgets.value != "0") {
                    ...
                    if (imgets.value != "0")
                        ...
                        tmpshift2=mktemp("tmpshift2")
                        xregister(in[i]//"["//j//"]"//"[*,*,3]",tmprefimage,
                            l_regions,tmpregister,output=tmpshift1,interac-,
                            verbose-)
                        delete(tmpregister,ver-, >& "dev$null")
                        xregister(in[i]//"["//j+1//"]"//"[*,*,3]",tmprefimage,
                            l_regions,tmpregister,output=tmpshift2,interac-,
                            verbose-)
                        delete(tmpregister,ver-, >& "dev$null")
                        imarith(tmpshift1,"+",tmpshift2,tmpfile2,ver-)
                        if (l_combine == "average")
