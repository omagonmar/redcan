...
begin
    ...
    while (fscan(scanfile,l_filename) != EOF) {
        ...
        if (!imaccess(l_rawpath//l_filename))
            glogprint( l_logfile, "tprepare", "status", type="error", errno=101,
    ...
    while (i <= nimages) {
        ...
        glogprint( l_logfile, "tprepare", "status", type="fork", fork="forward",
        ...
        if (l_check && j != 0) {
            glogprint( l_logfile, "tprepare", "status", type="error", errno=123,
                str="Image "//in[i]//" does not have the expected structure.",
            verbose+)
            status=1
            goto nextimage
        }
        ...
        if (filetype != 1) {
            glogprint( l_logfile, "tprepare", "status", type="error", errno=121,
                str="Image "//in[i]//" appears to be a 'prepared' T-ReCS file.",
                verbose+)
            status=1
            goto nextimage
        }
        ...
        if (i_frames == 0) {
            glogprint( l_logfile, "tprepare", "status", type="error", errno=123,
                str="Either zero nod sets or zero save sets in image "//\
                in[i]//".",verbose+)
            status=1
            goto nextimage
        }
        ...
        for (j=1; j <= lext; j=j+1) {
            ...
            if (modeflag == 1 || modeflag == 3) {
                ...
                if (imgets.value == "0") {
                    ...
                    i_nodset=int(imgets.value)
                    if (i_nodset < 1) {
                        gloginit( l_logfile, "tprepare", "status", type="error",
                            errno=121, str="Nod number recorded as "//\
                            imgets.value//".",verbose+)
                        status=1
                        goto nextimage
            ...
            if (modeflag != 4) {
                delete( tmplog, verify-, >& "dev$null" )
                imcombine("@"//tmpfile,tmpoff,headers="",bpmasks="",rejmasks="",
                    nrejmasks="",expmasks="",sigmas="",logfile="STDOUT",
                    combine=l_combine,reject="none",project=no,outtype="double",
                    outlimits="", weight="none", offsets="none", scale="none",
                    >& tmplog )
                ...
                if (l_stack == "register" && iregister != 0) {
                    xregister("@"//tmplist,refimage,"[*,*]",tmpregister,
                        output="@"//tmpshift,interac-,verbose-)
            ...
            if (j == 1) {
                ...
                # KL shouldn't it be add+, or addonly+???
                hedit(out[i]//".fits","NEXTEND",i_extensions,add-,addonly-,
                    delete-,verify-,show-,update+)
            }
            l=j-1
            fxinsert(tmpfinal//".fits",out[i]//".fits["//str(l)//"]","",verb-)

            if (modeflag == 1 || modeflag == 3) {
                if (source == 1)
                    hedit(out[i]//".fits["//str(j)//"]","NOD","A",add+,addonly-,
                        delete-,verify-,show-,update+)
                else
                    hedit(out[i]//".fits["//str(j)//"]","NOD","B",add+,addonly-,
                        delete-,verify-,show-,update+)
                hedit(out[i]//".fits["//str(j)//"]","NODSET",nodcount,add+,
                    addonly-,delete-,verify-,show-,update+)
            }
            hedit(out[i]//".fits["//str(j)//"]","SAVESETS",i_saveset,add+,
                addonly-,delete-,verify-,show-,update+)
            imgets(exheader,"UTSTART", >& "dev$null")
            if (imgets.value != "0")
                hedit(out[i]//".fits["//str(j)//"]","UTSTART",imgets.value,
                    add+,addonly-,delete-,verify-,show-,update+)
            imgets(exheader,"UTEND", >& "dev$null")
            if (imgets.value != "0")
                hedit(out[i]//".fits["//str(j)//"]","UTEND",imgets.value,add+,
                    addonly-,delete-,verify-,show-,update+)
            imgets(exheader,"AMSTART", >& "dev$null")
            if (imgets.value != "0") {
                airmass=real(imgets.value)
                hedit(out[i]//".fits["//str(j)//"]","AMSTART",airmass,add+,
                    addonly-,delete-,verify-,show-,update+)
            }

            hedit(out[i]//".fits["//str(j)//"]","AXISLAB4","",add-,addonly-,
                delete+,update+,verify-,show-)
        ...
        # The following is a patch to add spectroscopy keywords that Tracy needs
        # for her routines
        ...
        nhedit (out[i]//"[0]", "GEM-TLM", gemdate.outdate,
            "UT Last modification with GEMINI IRAF", comfile="NULL", after="",
