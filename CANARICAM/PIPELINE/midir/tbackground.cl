    ...
    while (l <= nimages) {
        ...
        if ((tstat.vmin < (tstat.mean-tstat.stddev*l_sigma)) || (tstat.vmax > (tstat.mean+tstat.stddev*l_sigma))) {
            ...
            for (k=1;k<= nnodsets;k+=1) {
                ...
                if (m > nbadsetorig) {
                    hedit(l_image//"["//str(2*k-1)//"]","NBADSET",m,add=yes,
                        addonly=no,delete=no,verify=no,show=l_sh_change,
                        update=yes)
                    if (modeflag == 1) {
                        hedit(l_image//"["//str(2*k)//"]","NBADSET",m,add=yes,
                            addonly=no,delete=no,verify=no,show=l_sh_change,
                            update=yes)
                    }
                    for (i=1;i <= m;i+=1) {
                        if (i < 10) {
                            hedit(l_image//"["//str(2*k-1)//"]",
                                "BADSET0"//str(i),str(badsetorig[i]),add=yes,
                                addonly=no,delete=no,verify=no,show=l_sh_change,
                                update=yes)
                            if (modeflag == 1)
                                hedit(l_image//"["//str(2*k)//"]",
                                    "BADSET0"//str(i),str(badsetorig[i]),
                                    add=yes,addonly=no,delete=no,verify=no,
                                    show=l_sh_change,update=yes)
                        } else {
                            hedit(l_image//"["//str(2*k-1)//"]",
                                "BADSET"//str(i),str(badsetorig[i]),add=yes,
                                addonly=no,delete=no,verify=no,show=l_sh_change,
                                update=yes)
                            if (modeflag == 1)
                                hedit(l_image//"["//str(2*k)//"]",
        ...
        #
        gemdate ()
        nhedit (l_image//"[0]", "TBACKGRO", gemdate.outdate,
            "UT Time stamp for TBACKGROUND", comfile="NULL", after="",
            before="", update+, add+, addonly-, delete-, verify-, show-)
        nhedit (l_image//"[0]", "GEM-TLM", gemdate.outdate,
