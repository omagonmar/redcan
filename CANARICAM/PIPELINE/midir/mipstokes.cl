...
begin
    ...
    if (l_outpref != "") {
        ...
        if (status != 0) {
            glogprint (l_logfile, "mipstokes", "status", type="warning",
                errno=0, str="Resetting 'outpref' to '"//l_outpref//"' because \
                'frametype' not 'dif'", verbose=l_verbose)
        }
    }
    ...
    if ((gemextn.fail_count > 0) || (nimages == 0) || \
        ...
        glogprint (l_logfile, "mipstokes", "status", type="error", errno=status,
            str=errmsg, verbose+)
        goto clean
    } else {
        scanfile = tmpinimg
    ...
    for (i=1; i<=nimages; i=i+1) {
        ...
        tail (tmpfile, nlines=1) | scan (nextensions)
        delete (tmpfile, ver-, >& "dev$null")
        ...
        if (modeflag == 1) {
            ...
            if (nnods == 1) {
                ...
                status = 123
                errmsg = "Number of extensions does not correspond to complete \
        ...
        if (modeflag == 1) {
            ...
            for (nod=1; nod<=nnods; nod=nod+2) {
                ...
                glogprint (l_logfile, "mipstokes", "engineering", type="string",
                    str="Working on nods "//str(nod)//" and "//str(nod+1),
                    verbose=l_verbose)
                ...
                for (ext=8*(nod-1)+1; ext<=8*nod; ext=ext+1) {
                    ...
                    if (l_fl_stair) {
                        for (coord=1; coord<=301; coord=coord+20) {
                            ...
                            imarith (tmpreg1//imgsec, "-", chlevel, tmpimg1,
                                ...
                                calctype="", verb-, noact-)
                            imcopy (tmpimg1, tmpreg1//imgsec, ver-)
                            imdelete (tmpimg1, ver-, >& "dev$null")
                            ...
                            imarith (tmpreg2//imgsec, "-", chlevel, tmpimg1,
                                title="", divzero=0., hparams="", pixtype="",
                                calctype="", verb-, noact-)
                            imcopy (tmpimg1, tmpreg2//imgsec, ver-)
                            imdelete (tmpimg1, ver-, >& "dev$null")
                        }                            
                    }
                    
                    if (l_fl_register) {
                        xregister (tmpreg2, tmpreg1, regions=l_regions,
                            shifts=tmpshifts, output=tmpworkimg, append+,
                            records="", coords="", xlag=0, ylag=0, dxlag=0,
                            dylag=0, background="none", border=INDEF,
                        ...
                        imarith (tmpreg1, "+", tmpworkimg, tmpworkimg, title="",
                    ...
                    imdelete (tmpreg1, ver-, >& "dev$null")
                    imdelete (tmpreg2, ver-, >& "dev$null")
                    ...
                    if ((k == 1) || (k == 4)) {     # pair 1
                        if (l_fl_register) {
                            if (flag[1] == 0) {
                                flag[1] = 1
                                imcopy (tmpworkimg, tmprefer1, ver-)
                                imcopy (tmpworkimg, tmpims1a, ver-)
                            } else {
                                tmpreg1 = mktemp ("tmpreg1")
                                xregister (tmpworkimg, tmprefer1,
                                    regions=l_regions, shifts=tmpshifts,
                                    output=tmpreg1, append+, records="",
                                    coords="", xlag=0, ylag=0, dxlag=0, dylag=0,
                                    background="none", border=INDEF,
                                    loreject=INDEF, hireject=INDEF, apodize=0.,
                                    filter="none", correlation="discrete",
                                    xwindow=11, ywindow=11, function="centroid",
                                    xcbox=11, ycbox=11, interp_type="poly5",
                                    boundary_typ="nearest", constant=0.,
                                    interact-, verb+)
                                if (k == 1)
                                    imcopy (tmpreg1, tmpims1a, ver-)
                                else if (k == 4)
                                    imcopy (tmpreg1, tmpims1b, ver-)
                                imdelete (tmpreg1, ver-, >& "dev$null")
                            }
                        } else {
                            if (k == 1)
                                ...
                                imcopy (tmpworkimg, tmpims1b, ver-)
                        }
                    } else if ((k == 2) || (k == 3)) {  # pair 2
                        if (l_fl_register) {
                            if (flag[2] == 0) {
                                flag[2] = 1
                                imcopy (tmpworkimg, tmprefer2, ver-)
                                imcopy (tmpworkimg, tmpims2a, ver-)
                            } else {
                                tmpreg1 = mktemp ("tmpreg1")
                                xregister (tmpworkimg, tmprefer2,
                                    regions=l_regions, shifts=tmpshifts,
                                    output=tmpreg1, append+, records="",
                                    coords="", xlag=0, ylag=0, dxlag=0, dylag=0,
                                    background="none", border=INDEF,
                                    ...
                                    interact-, verb+)
                                if (k == 2)
                                    imcopy (tmpreg1, tmpims2a, ver-)
                                else if (k == 3)
                                    imcopy (tmpreg1, tmpims2b, ver-)
                                imdelete (tmpreg1, ver-, >& "dev$null")
                            }
                        } else {
                            if (k == 2)
                                ...
                                imcopy (tmpworkimg, tmpims2b, ver-)
                        }
                    } else if ((k == 5) || (k == 8)) {  # pair 3
                        if (l_fl_register) {
                            if (flag[3] == 0) {
                                flag[3] = 1
                                imcopy (tmpworkimg, tmprefer3, ver-)
                                imcopy (tmpworkimg, tmpims3a, ver-)
                            } else {
                                tmpreg1 = mktemp ("tmpreg1")
                                xregister (tmpworkimg, tmprefer3,
                                    regions=l_regions, shifts=tmpshifts,
                                    output=tmpreg1, append+, records="",
                                    coords="", xlag=0, ylag=0, dxlag=0, dylag=0,
                                    background="none", border=INDEF,
                                    ...
                                    interact-, verbose+)
                                if (k == 5)
                                    imcopy (tmpreg1, tmpims3a, ver-)
                                else if (k == 8)
                                    imcopy (tmpreg1, tmpims3b, ver-)
                                imdelete (tmpreg1, ver-, >& "dev$null")
                            }
                        } else {
                            if (k == 5)
                                ...
                                imcopy (tmpworkimg, tmpims3b, ver-)
                        }
                    } else if ((k == 6) || (k == 7)) {  # pair 4
                        if (l_fl_register) {
                            if (flag[4] == 0) {
                                flag[4] = 1
                                imcopy (tmpworkimg, tmprefer4, ver-)
                                imcopy (tmpworkimg, tmpims4a, ver-)
                            } else {
                                tmpreg1 = mktemp ("tmpreg1")
                                xregister (tmpworkimg, tmprefer4,
                                    regions=l_regions, shifts=tmpshifts,
                                    output=tmpreg1, append+, records="",
                                    coords="", xlag=0, ylag=0, dxlag=0, dylag=0,
                                    background="none", border=INDEF,
                                    ...
                                    interact-, verbose+)
                                if (k == 6)
                                    imcopy (tmpreg1, tmpims4a, ver-)
                                else if (k == 7)
                                    imcopy (tmpreg1, tmpims4b, ver-)
                                imdelete (tmpreg1, ver-, >& "dev$null")
                            }
                        } else {
                            if (k == 6)
                                ...
                                imcopy (tmpworkimg, tmpims4b, ver-)
                        }
                    } else {
                        status = 99
                ...
                imdelete (tmpims1a, ver-, >& "dev$null")
                imdelete (tmpims1b, ver-, >& "dev$null")
                imdelete (tmpims2a, ver-, >& "dev$null")
                imdelete (tmpims2b, ver-, >& "dev$null")
                imdelete (tmpims3a, ver-, >& "dev$null")
                imdelete (tmpims3b, ver-, >& "dev$null")
                imdelete (tmpims4a, ver-, >& "dev$null")
                imdelete (tmpims4b, ver-, >& "dev$null")
               
                if (l_fl_register) {
                    tmpreg1 = mktemp ("tmpreg1")
                    xregister (tmpimg2, tmpimg1, regions=l_regions,
                        shifts=tmpshifts, output=tmpreg1, append+, records="",
                        coords="", xlag=0, ylag=0, dxlag=0, dylag=0,
                        ...
                        constant=0., interact-, verbose+)
                    imdelete (tmpimg2, ver-, >& "dev$null")
                    imcopy (tmpreg1, tmpimg2, ver-)
                    imdelete (tmpreg1, ver-, >& "dev$null")
                    
                    tmpreg1 = mktemp ("tmpreg1")
                    xregister (tmpimg3, tmpimg1, regions=l_regions,
                        shifts=tmpshifts, output=tmpreg1, append+, records="",
                        coords="", xlag=0, ylag=0, dxlag=0, dylag=0,
                        background="none", border=INDEF, loreject=INDEF,
                        hireject=INDEF, apodize=0., filter="none",
                        ...
                        interp_type="poly5", boundary_typ="nearest",
                        constant=0., interact-, verbose+)
                    imdelete (tmpimg3, ver-, >& "dev$null")
                    imcopy (tmpreg1, tmpimg3, ver-)
                    imdelete (tmpreg1, ver-, >& "dev$null")
                    
                    tmpreg1 = mktemp ("tmpreg1")
                    xregister (tmpimg4, tmpimg1, regions=l_regions,
                        shifts=tmpshifts, output=tmpreg1, append+, records="",
                        coords="", xlag=0, ylag=0, dxlag=0, dylag=0,
                        background="none", border=INDEF, loreject=INDEF,
                        hireject=INDEF, apodize=0., filter="none", 
                        ...
                        interp_type="poly5", boundary_typ="nearest",
                        constant=0., interact-, verbose+)
                    imdelete (tmpimg4, ver-, >& "dev$null")
                    imcopy (tmpreg1, tmpimg4, ver-)
                    imdelete (tmpreg1, ver-, >& "dev$null")
                }
                ...
                imcombine (tmpimg1//","//tmpimg2//","//tmpimg3//","//tmpimg4,
                    ...
                    rdnoise="0.", gain="1.", snoise="0.1", sigscale=0.1,
                    pclip=-0.5, grow=0., >& "dev$null")
                imarith (tmpIstokes, "/", 2.0, tmpIstokes, title="", divzero=0.,
                ...
                if (l_fl_mask) {
                    ...
                    nmisc.fixpix (tmpQstokes, tmpimg1, linterp="", cinterp="",
                        ninterp="", creplace="1-10", constant=0.)
                    nmisc.fixpix (tmpUstokes, tmpimg1, linterp="", cinterp="",
                        ninterp="", creplace="1-10", constant=0.)
                    imdelete (tmpimg1, ver-, >& "dev$null")
                }
                ...
                tmpimg1 = mktemp ("tmpimg1")
                tmpimg2 = mktemp ("tmpimg2")
                imcopy (tmpUstokes, tmpimg1, ver-)
                imcopy (tmpQstokes, tmpimg2, ver-)
                ...
                if (nod == 1) {
                    ...
                    fxinsert (tmpIstokes, out[i]//"["//str(ext)//"]", "", verb-)
                    outextn += 1
                }
                firstextn = outextn
              
                fxinsert (tmpUstokes, out[i]//"["//str(outextn)//"]", "", verb-)
                outextn += 1
                fxinsert (tmpQstokes, out[i]//"["//str(outextn)//"]", "", verb-)
                outextn += 1
                fxinsert (tmpimg3, out[i]//"["//str(outextn)//"]", "", verb-)
                outextn += 1
                fxinsert (tmpimg1, out[i]//"["//str(outextn)//"]", "", verb-)
                outextn += 1
                fxinsert (tmpimg4, out[i]//"["//str(outextn)//"]", "", verb-)
                outextn += 1
                fxinsert (tmpimg2, out[i]//"["//str(outextn)//"]", "", verb-)
                outextn += 1
                lastextn = outextn                
                ...
                imdelete (tmpimg3, ver-, >& "dev$null")
                imdelete (tmpimg4, ver-, >& "dev$null")
                imdelete (tmpQstokes, ver-, >& "dev$null")
                imdelete (tmpUstokes, ver-, >& "dev$null")
                imdelete (tmpIstokes, ver-, >& "dev$null")
                
                # Propagate WCS information to the stacked images
                for (ext=firstextn; ext<=lastextn; ext=ext+1) {
                    nhedit (out[i]//"["//str(ext)//"]", "WAXMAP01", "", "",
                        comfile="NULL", after="", before="", update+, add-,
                        addonly-, delete+, verify-, show-)
                    
                    keyfound = ""
                    hselect (in[i]//"[0]", "CTYPE1", yes) | scan (keyfound)
                    if (keyfound != "") {
                        nhedit (out[i]//"["//str(ext)//"]", "WCSAXES", 2,
                            "Number of WCS axes in the image", comfile="NULL",
                            after="", before="", update+, add+, addonly-,
                            delete-, verify-, show-)
                        nhedit (out[i]//"["//str(ext)//"]", "CTYPE1",
                    ...
                    if (keyfound != "")
                        nhedit (out[i]//"["//str(ext)//"]", "CRPIX1",
                            keyfound, "Ref pix of axis 1", comfile="NULL",
                            after="", before="", update+, add+, addonly-,
                            delete-, verify-, show-)
                    ...
                    if (keyfound != "")
                        nhedit (out[i]//"["//str(ext)//"]", "CRPIX2",
                            keyfound, "Ref pix of axis 2", comfile="NULL",
                            after="", before="", update+, add+, addonly-,
                            delete-, verify-, show-)
                    ...
                    if (keyfound != "")
                        nhedit (out[i]//"["//str(ext)//"]", "CD1_1",
                            keyfound, "WCS matrix element 1 1", comfile="NULL",
                            after="", before="", update+, add+, addonly-,
                            delete-, verify-, show-)
                    ...
                    if (keyfound != "")
                        nhedit (out[i]//"["//str(ext)//"]", "CD1_2",
                            keyfound, "WCS matrix element 1 2", comfile="NULL",
                            after="", before="", update+, add+, addonly-,
                            delete-, verify-, show-)
                    ...
                    if (keyfound != "")
                        nhedit (out[i]//"["//str(ext)//"]", "CD2_1",
                            keyfound, "WCS matrix element 2 1", comfile="NULL",
                            after="", before="", update+, add+, addonly-,
                            delete-, verify-, show-)
                    ...
                    if (keyfound != "")
                        nhedit (out[i]//"["//str(ext)//"]", "CD2_2",
                            keyfound, "WCS matrix element 2 2", comfile="NULL",
                            after="", before="", update+, add+, addonly-,
                            delete-, verify-, show-)
                    ...
                    if (keyfound != "")
                        nhedit (out[i]//"["//str(ext)//"]", "RADECSYS",
                            keyfound, "R.A./DEC. coordinate system reference",
                            comfile="NULL", after="", before="", update+, add+, 
                            addonly-, delete-, verify-, show-)
                    
                    nhedit (out[i]//"["//str(ext)//"]", "EXTNAME", "SCI",
                        "Extension name", comfile="NULL", after="", before="",
                        update+, add+, addonly-, delete-, verify-, show-)
                    nhedit (out[i]//"["//str(ext)//"]", "EXTVER", ext,
                        "Extension number", comfile="NULL", after="", before="",
                        update+, add+, addonly-, delete-, verify-, show-)
            ...
            imdelete (tmprefer1, ver-, >& "dev$null")
            imdelete (tmprefer2, ver-, >& "dev$null")
            imdelete (tmprefer3, ver-, >& "dev$null")
            imdelete (tmprefer4, ver-, >& "dev$null")
        ...
        # Time stamps
        gemdate ()
        nhedit (out[i]//"[0]", "MIPSTOKE", gemdate.outdate,
            "UT Time stamp for MIPSTOKES", comfile="NULL", after="",
