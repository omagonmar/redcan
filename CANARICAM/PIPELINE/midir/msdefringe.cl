...
begin
    ...
    while (l1 == 0) {
        ...
        hedit (tmpscaled1, "CD1_1", 1., ver-, show-, update+)
        hedit (tmpscaled2, "CD1_1", 1., ver-, show-, update+)
        ...
        if (l1 != 0) {
            ...
            if (keyfound != "") {
                hedit (tmp3//"r.fits", "OBJECT", keyfound, add+, addonly-, 
                    verify-, delete-, show-, update+)
                hedit (tmp3//"i.fits", "OBJECT", keyfound, add+, addonly-, 
                    verify-, delete-, show-, update+)
            }
            if (l_fl_mef) {
    ...
    gemdate ()
    if (l_fl_mef) {
        nhedit (out//"[0]",  "MSDEFRINGE",  gemdate.outdate, 
            "UT Time stamp for MSDEFRINGE",  comfile="NULL",  after="", 
            before="",  update+,  add+,  addonly-,  delete-,  verify-,  show-)
        nhedit (out//"[0]",  "GEM-TLM",  gemdate.outdate, 
            "UT Last modification with GEMINI",  comfile="NULL",  after="", 
            before="",  update+,  add+,  addonly-,  delete-,  verify-,  show-)
    }
    else {
        nhedit (out,  "MSDEFRINGE",  gemdate.outdate, 
            "UT Time stamp for MSDEFRINGE",  comfile="NULL",  after="", 
            before="",  update+,  add+,  addonly-,  delete-,  verify-,  show-)
        nhedit (out,  "GEM-TLM",  gemdate.outdate, 
            "UT Last modification with GEMINI",  comfile="NULL",  after="", 
            before="",  update+,  add+,  addonly-,  delete-,  verify-,  show-)
    }
