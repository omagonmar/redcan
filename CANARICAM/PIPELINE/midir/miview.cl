    ...
    while(i<=nimages) {
	    ...
	    if ((instrument == "TReCS") || (instrument == "CanariCam")) {
                ...
                if (imgets.value == "0") {
                    ...
                    glogprint( l_logfile, "miview", "status", type="string",
                        str="Displaying TReCS/CanariCam images.", verbose=l_verbose)
            ...
            if (modeflag == 1 || modeflag==2) {
                ...
                if (l_type == "src")
                    ...
                    glogprint( l_logfile, "miview", "status", type="warning",
                        errno=99, str="'sig' type not implemented, using 'dif'",
                        verbose+)
                    ndim=3
                } 
            }
                    ...
                    while (npars == 0) {
                        ...
                        if (fscan(imcur,xpos,ypos,wcs,cursinput) != EOF) {
                            ...
                            if (cursinput == "u" || cursinput == "U") {
                                glogprint( l_logfile, "miview", "science",
                                    type="string", str="Nod "//n_i//" unmarked \
                                    as bad.",verbose=l_verbose)
                                hedit(out[i]//"["//str(n_i)//"]","BADNOD","",
                                    add=no,addonly=no,delete=yes,verify=no,
                                    show=no,update=yes)
                                nbadsets=nbadsets-1
                                npars=0
                            }
        ...
        if (check != "0") {
            ...
            if (!l_manual) {
                ...
                display (in[i]//"["//str(n_i)//"][*,*]",1,erase-,zscale=lzscale,
                    z1=lz1,z2=lz2,ztrans=lztrans,zrange=lzrange,fill=l_dispfill,
                    >& "dev$null")
            }
            ...
            if (l_manual) {
                ...
                fxinsert(tmpfile1//".fits",out[i]//"["//n_i-1//"]",
                    groups="",ver-)
                imdelete(tmpfile1,ver-,>& "dev$null")
            }
                    ...
                    if (fscan(imcur,xpos,ypos,wcs,cursinput) != EOF) {
                        ...
                        if (cursinput == "u" || cursinput == "U") {
                            glogprint( l_logfile, "miview", "science",
                                ...
                                bad.", verbose=l_verbose)
                            hedit(out[i]//"["//str(n_i)//"]","BADNOD","",add=no,
        ...
        if (l_manual) {
            if (nbadsets == 0) {
                ...
                nhedit (out[i]//"[0]", "GEM-TLM", gemdate.outdate,
                    "UT Last modification with GEMINI IRAF", comfile="NULL",
                    after="", before="", update+, add+, addonly-, delete-,
                    verify-, show-)
                nhedit (out[i]//"[0]", "MIVIEW", gemdate.outdate,
                    "UT Time stamp for MIVIEW", comfile="NULL", after="",
