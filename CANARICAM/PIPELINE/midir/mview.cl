...
begin
    ...
    while (i<=nimages) {
        ...
        if (modeflag == 1 || modeflag==2) {
            ...
            if ((l_type!="sig")&&(l_type!="ref")&&(l_type!="dif")&&(l_type!="src")) {
                glogprint( l_logfile, "mview", "status", type="error",
                    errno=123, str="Images type invalid.  lpar mview for valid \
                    values.",verbose+)    
                status=1
                goto clean
            }
                    ...
                    if (fscan(imcur,xpos,ypos,wcs,cursinput) != EOF) {
                        ...
                        if (cursinput == "u" || cursinput == "U") {
                            ...
                            hedit(out[i]//"["//str(n_i)//"]","BADNOD",add=no,
                                addonly=no,delete=yes,verify=no,show=yes,
                                update=yes)
                            nbadsets=nbadsets-1
                            npars=0
                        }
        ...
        if (l_manual) {
            if (nbadsets == 0) {
                ...
                nhedit (out[i]//"[0]", "GEM-TLM", gemdate.outdate,
                    ...
                    verify-, show-)
                nhedit (out[i]//"[0]", "MVIEW", gemdate.outdate,
                    "UT Time stamp for MVIEW", comfile="NULL", after="",
