...
begin
    ...
    while (i <= nimages) {
        ...
        if (!l_fl_register) {
            ...
            tmp5 = mktemp ("tmp5")
            tmp6 = mktemp ("tmp6")
            xregister (in[i]//"[2]", in[i]//"[1]", regions=l_regions,
                shifts=tmpshifts, output=tmp4, background="none",
                loreject=INDEF, hireject=INDEF, apodize=0., filter="none",
                append+, records="", correlation="discrete", xwindow=11,
                ywindow=11, xcbox=11, ycbox=11, function="centroid",
                interp_type="poly5", interact-, xlag=0, ylag=0, dxlag=0,
                dylag=0)
            xregister (in[i]//"[3]", in[i]//"[1]", regions=l_regions,
                shifts=tmpshifts, output=tmp5, background="none",
                loreject=INDEF, hireject=INDEF, apodize=0., filter="none",
                append+, records="", correlation="discrete", xwindow=11,
                ywindow=11, xcbox=11, ycbox=11, function="centroid",
                interp_type="poly5", interact-, xlag=0, ylag=0, dxlag=0,
                dylag=0)
            xregister (in[i]//"[4]", in[i]//"[1]", regions=l_regions,
                shifts=tmpshifts, output=tmp6, background="none",
                loreject=INDEF, hireject=INDEF, apodize=0., filter="none",
                append+, records="", correlation="discrete", xwindow=11,
                ywindow=11, xcbox=11, ycbox=11, function="centroid",
                interp_type="poly5", interact-, xlag=0, ylag=0, dxlag=0,
            ...
            imdelete (tmp5, ver-, >& "dev$null")
            imdelete (tmp6, ver-, >& "dev$null")
        }
        wmef (tmp3, out[i], extname="SCI", phu=header, verb-, >& "dev$null")
        fxinsert (tmp2, out[i]//"["//j//"]", "", verbose=l_verbose,
            >& "dev$null")
        j = j+1
        fxinsert (tmp1, out[i]//"["//j//"]", "", verbose=l_verbose,
            >& "dev$null")
        imdelete (tmp1//","//tmp2//","//tmp3, ver-, >& "dev$null")
        ...
        if (keyfound != "") {
            for (k=1; k<=3; k=k+1) {
                nhedit (out[i]//"["//str(k)//"]", "WCSAXES", 2,
                    comment="Number of WCS axes in the image", comfile="NULL",
                    after="", before="", update+, add+, addonly-, delete-,
                    verify-, show-)
                nhedit (out[i]//"["//str(k)//"]", "CTYPE1", keyfound,
                    comment="R.A. in tangent plane projection", comfile="NULL",
                    after="", before="", update+, add+, addonly-, delete-,
                    verify-, show-)
            }
        }
        ...
        if (keyfound != "") {
            for (k=1; k<=3; k=k+1) {
                nhedit (out[i]//"["//str(k)//"]", "CRVAL1", keyfound,
                    comment="RA at Ref pix in decimal degrees", comfile="NULL",
                    after="", before="", update+, add+, addonly-, delete-,
                    verify-, show-)
            }
        }
        ...
        if (keyfound != "") {
            for (k=1; k<=3; k=k+1) {
                nhedit (out[i]//"["//str(k)//"]", "CTYPE2", keyfound,
                    comment="DEC. in tangent plane projection", comfile="NULL",
                    after="", before="", update+, add+, addonly-, delete-,
                    verify-, show-)
            }
        }
        ...
        if (keyfound != "") {
            for (k=1; k<=3; k=k+1) {
                nhedit (out[i]//"["//str(k)//"]", "CRPIX2", keyfound,
                    comment="Ref pix of axis 2", comfile="NULL", after="",
        ...
        if (keyfound != "") {
            for (k=1; k<=3; k=k+1) {
                nhedit (out[i]//"["//str(k)//"]", "CRVAL2", keyfound,
                    comment="DEC at Ref pix in decimal degrees", comfile="NULL",
                    after="", before="", update+, add+, addonly-, delete-,
                    verify-, show-)
            }
        }
        ...
        if (keyfound != "") {
            for (k=1; k<=3; k=k+1) {
                nhedit (out[i]//"["//str(k)//"]", "CD1_1", keyfound,
                    comment="WCS matrix element 1 1", comfile="NULL", after="",
                    before="", update+, add+, addonly-, delete-, verify-, show-)
            }
        }
        keyfound = ""
        ...
        if (keyfound != "") {
            for (k=1; k<=3; k=k+1) {
                nhedit (out[i]//"["//str(k)//"]", "CD1_2", keyfound,
                    comment="WCS matrix element 1 2", comfile="NULL", after="",
                    before="", update+, add+, addonly-, delete-, verify-, show-)
            }
        }
        keyfound = ""
        ...
        if (keyfound != "") {
            for (k=1; k<=3; k=k+1) {
                nhedit (out[i]//"["//str(k)//"]", "CD2_1", keyfound,
                    comment="WCS matrix element 2 1", comfile="NULL", after="",
                    before="", update+, add+, addonly-, delete-, verify-, show-)
            }
        }
        keyfound = ""
        ...
        if (keyfound != "") {
            for (k=1; k<=3; k=k+1) {
                nhedit (out[i]//"["//str(k)//"]", "CD2_2", keyfound,
                    comment="WCS matrix element 2 2", comfile="NULL", after="",
                    before="", update+, add+, addonly-, delete-, verify-, show-)
            }
        }
        keyfound = ""
        ...
        for (k=1; k<=3; k=k+1) {
            nhedit (out[i]//"["//str(k)//"]", "EXTNAME", "SCI",
                comment="Extension name", comfile="NULL", after="", before="",
        ...
        # Time stamps
        gemdate ()
        nhedit (out[i]//"[0]", "MIPTRANS", gemdate.outdate,
            "UT Time stamp for MIPTRANS", comfile="NULL", after="",
            before="", update+, add+, addonly-, delete-, verify-, show-)
