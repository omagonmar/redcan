...
#            status = 123
#            errmsg = "Source "//in[i]//" not "//nsheaders.instrument//" data.  \
#                Skipping."
#            glogprint (l_logfile, "msreduce", "status", type="warning",
#                errno=status, str=errmsg, verbose=l_verbose)
#            nskipped += 1
...
#        }
        ...
        if (l_fl_flat) {
            ...
            if (sinstrument != instrument) {
                tmpstatus = 123
                errmsg = "Flat file "//flatim[i]//" not "//instrument//" data.  \
                    Skipping."
                glogprint (l_logfile, "msreduce", "status", type="warning",
...
#            }
            ...
            if (sinstrument != instrument) {
                tmpstatus = 123
                errmsg = "Bias file "//biasim[i]//" not "//instrument//" data.  \
                    Skipping."
                glogprint (l_logfile, "msreduce", "status", type="warning",
                    errno=tmpstatus, str=errmsg, verbose=l_verbose)
            }
...
#            }
            ...
            if (tmpstatus != 0) {
                errmsg = "Flat "//flatim[i]//" and/or bias "//biasim[i]//" not \
                    a spectrum."
                glogprint (l_logfile, "msreduce", "status", type="warning",
                    errno=status, str=errmsg, verbose=l_verbose)
            }
...
#            }      
        ...
        if (no == l_fl_reextract) {
            ...
            if (l_fl_process) {
                ...
                mireduce (source, outimages="r"//source, outpref="",
                    rawpath=curpath, fl_background-, fl_view-, fl_mask-, bpm="",
                    fl_flat-, flatfieldfile="", stackoption="stack",
                ...
                if (l_fl_std) {
                    ...
                    mireduce (reference, outimages="r"//reference, outpref="",
                        ...
                        stackoption="stack", frametype="dif", combine="average",
                        fl_display=no, fl_check=yes, fl_rescue=no,
                        region="[*,*]", fl_variance=no, logfile=l_logfile,
                        verbose=l_verbose)
                    ...
                    mireduce (reference, outimages="a"//reference, outpref="",
                        rawpath=curpath, fl_background=no, fl_view=no,
                        fl_mask=no, bpm="", fl_flat=no, flatfieldfile="",
                        stackoption="stack", frametype="ref", combine="average",
                        fl_display=no, fl_check=yes, fl_rescue=no,
                        region="[*,*]", fl_variance=no, logfile=l_logfile,
                        verbose=l_verbose)
                }
        ...
        nsextract ("tfr"//source, outspectra="xtfr"//source, outpref="",
            dispaxis=1, database="", line=l_line, nsum=10, ylevel=0.1,
            upper=10., lower=-10., background="average", fl_addvar+,
            fl_skylines+, fl_inter=l_fl_extract, fl_apall+,
            fl_trace=l_fl_retrace, aptable="gnirs$data/apertures.fits",
            fl_flipped+, fl_project-, fl_findneg-, bgsample="*", trace="",
            tr_nsum=10, tr_step=10, tr_nlost=3, tr_function="legendre",
            tr_order=5, tr_sample="*", tr_naver=1, tr_niter=1, tr_lowrej=3.,
            tr_highrej=3., tr_grow=0., logfile=l_logfile, verbose=l_verbose)
        if (l_fl_std) {
            imdelete ("xtfr"//reference, verify-, >& "dev$null")
            nsextract ("tfr"//reference, outspectra="xtfr"//reference, outpref="",
                dispaxis=1, database="", line=l_line, nsum=10, ylevel=0.1,
                upper=10., lower=-10., background="average", fl_addvar+,
                fl_skylines+, fl_inter=l_fl_extract, fl_apall+,
                fl_trace=l_fl_retrace, aptable="gnirs$data/apertures.fits",
        ...
        if (l_fl_negative) {
            ...
            nsextract (tmpneg, outspectra="ntfr"//source, outprefix="",
                database="", dispaxis=1, line=l_line, nsum=10, ylevel=0.1,
                upper=10., lower=-10., background="average", fl_addvar+,
                fl_skylines+, fl_inter=l_fl_extract, fl_apall+,
                fl_trace=l_fl_retrace, aptable="gnirs$data/apertures.fits",
                fl_flipped+, fl_project-, fl_findneg-, bgsample="*", trace="", 
            ...
            if (l_fl_std) {
                ...
                nsextract (tmpneg, outspectra="ntfr"//reference, outprefix="",
                    ...
                    tr_grow=0., logfile=l_logfile, verbose=l_verbose)
                imdelete (tmpneg, verify-, >& "dev$null")
            }
            ...
            imsum ("ptfr"//source//"[1]","ntfr"//source//"[1]", tmpwork, title="",
                hparams="", pixtype="", calctype="", option="sum",
            ...
            if (l_fl_std) {
                ...
                imrename ("xtfr"//reference, "ptfr"//reference, ver-,
                    >& "dev$null")
                imsum ("ptfr"//reference//"[1],ntfr"//reference//"[1]", tmpwork,
        ...
        if (l_fl_std) {
            ...
            gemdate ()
            nhedit ("a"//prefix//source//".fits[0]", "MSREDUCE",
                gemdate.outdate, "UT Time stamp for MSREDUCE", comfile="NULL",
                after="", before="", update+, add+, addonly-, delete-, verify-,
                show-)
            nhedit ("a"//prefix//source//".fits[0]", "GEM-TLM", gemdate.outdate,
                "UT Last modification with GEMINI", comfile="NULL", after="",
                before="", update+, add+, addonly-, delete-, verify-, show-)
        } else {
            gemdate ()
            nhedit (prefix//source//".fits[0]", "MSREDUCE",
                gemdate.outdate, "UT Time stamp for MSREDUCE", comfile="NULL",
                after="", before="", update+, add+, addonly-, delete-, verify-,
                show-)
            nhedit (prefix//source//".fits[0]", "GEM-TLM", gemdate.outdate,
                "UT Last modification with GEMINI", comfile="NULL", after="",
                before="", update+, add+, addonly-, delete-, verify-, show-)
        }
