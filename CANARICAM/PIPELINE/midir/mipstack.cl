...
begin
    ...
    while (i <= nimages) {
        ...
        if (j == 0 && k == 0) {
            glogprint (l_logfile, "mipstack", "status", type="error",
                errno=142, str="Error: input file "//in[i]//" is not a polarimetry file",verbose+)
            status = status+1
            goto nextimage
        }
            ...
            if (nnods != 1) {
                for (m=1; m <= nnods; m=m+2) {
                    for (j=8*(m-1)+1; j <= 8*m ; j=j+1) {
                        ...
                        if (l_fl_register) {
                          ...
                          imcopy(in[i]//"["//j//"]"//"[*,*,"//framevalue//"].fits",tmpregister1,ver-, >& "dev$null")
                          imcopy(in[i]//"["//j+8//"]"//"[*,*,"//framevalue//"].fits",tmpregister2,ver-, >& "dev$null")
                          xregister(tmpregister2,tmpregister1,regions=l_regions,shifts=tmpshifts,output=tmpfile2,
                            background="none",loreject=INDEF,hireject=INDEF,apodize=0.,filter="none",
                            append+,records="",correlation="discrete",xwindow=11,ywindow=11,xcbox=11,ycbox=11,
                            function="centroid",interp_type="poly5",interact-,xlag=0,ylag=0,dxlag=0,dylag=0)
                          imarith(tmpregister1,"+",tmpfile2,tmpfile2,title="", divzero=0., hparams="",
                            pixtype="", calctype="", verbose-, noact-)
                          imdelete(tmpregister1//","//tmpregister2,ver-, >& "dev$null")
                        }
                        else {
                          imarith(in[i]//"["//j//"]"//"[*,*,"//framevalue//"].fits",
	        	      "+",in[i]//"["//j+8//"]"//"[*,*,"//framevalue//"].fits",
		            tmpfile2,title="", divzero=0., hparams="",
                            pixtype="", calctype="", verbose-, noact-)
                        }
                        ...
                        if (k == 1 || k == 4) {
                          if (l_fl_register) {
                            if (flag1 == 0) {
                              ...
                              tmpregister1=mktemp("tmpregister1")
                              xregister(tmpfile1,refim1,regions=l_regions,shifts=tmpshifts,output=tmpregister1,
                                background="none",loreject=INDEF,hireject=INDEF,apodize=0.,filter="none",
                                append+,records="",correlation="discrete",xwindow=11,ywindow=11,xcbox=11,ycbox=11,
                                function="centroid",interp_type="poly5",interact-,xlag=0,ylag=0,dxlag=0,dylag=0)
                        ...
                        if (k == 2 || k == 3) {
                          if (l_fl_register) {
                            if (flag2 == 0) {
                              ...
                              tmpregister1=mktemp("tmpregister1")
                              xregister(tmpfile1,refim2,regions=l_regions,shifts=tmpshifts,output=tmpregister1,
                                background="none",loreject=INDEF,hireject=INDEF,apodize=0.,filter="none",
                                append+,records="",correlation="discrete",xwindow=11,ywindow=11,xcbox=11,ycbox=11,
                                function="centroid",interp_type="poly5",interact-,xlag=0,ylag=0,dxlag=0,dylag=0)
                        ...
                        if (k == 5 || k == 8) {
                          if (l_fl_register) {
                            if (flag3 == 0) {
                              ...
                              tmpregister1=mktemp("tmpregister1")
                              xregister(tmpfile1,refim3,regions=l_regions,shifts=tmpshifts,output=tmpregister1,
                                background="none",loreject=INDEF,hireject=INDEF,apodize=0.,filter="none",
                                append+,records="",correlation="discrete",xwindow=11,ywindow=11,xcbox=11,ycbox=11,
                                function="centroid",interp_type="poly5",interact-,xlag=0,ylag=0,dxlag=0,dylag=0)
                        ...
                        if (k == 6 || k == 7) {
                          if (l_fl_register) {
                            if (flag4 == 0) {
                              ...
                              tmpregister1=mktemp("tmpregister1")
                              xregister(tmpfile1,refim4,regions=l_regions,shifts=tmpshifts,output=tmpregister1,
                                background="none",loreject=INDEF,hireject=INDEF,apodize=0.,filter="none",
                                append+,records="",correlation="discrete",xwindow=11,ywindow=11,xcbox=11,ycbox=11,
                                function="centroid",interp_type="poly5",interact-,xlag=0,ylag=0,dxlag=0,dylag=0)
                ...
                if (l_fl_stair) {
                  for (m=1; m <= 301; m=m+20) {
                    imstatistics(tmpfinal//"["//str(m)//":"//str(m+19)//",1:240]",
                      fields="midpt",lower=INDEF,upper=INDEF,format-,nclip=0,lsigma=3.,
                      usigma=3.,binwidth=0.1,cache-) | scanf("%f",chlevel)
                    imarith(tmpfinal//"["//str(m)//":"//str(m+19)//",1:240]","-",chlevel,
                      tmpwork)
                    imcopy(tmpwork,tmpfinal//"["//str(m)//":"//str(m+19)//",1:240]",ver-, >& "dev$null")
                ...
                for (k=1; k<=4; k=k+1) {
                    nhedit (out[i]//"["//k//"]", "WPLATE", wplate[k], 
                        comment="Waveplate angle", comfile="NULL", after="",
                        before="", update+, add+, addonly-, delete-, verify-,
                        show-)
                    nhedit (out[i]//"["//k//"]", "POLANGLE", polangle[k],
                        comment="Polarization selection angle", comfile="NULL",
                        after="", before="", update+, add+, addonly-, delete-,
                        verify-, show-)
                }
                ...
                for (k=1; k<=4; k=k+1) {
                    nhedit (out[i]//"["//k//"]", "ANGROT", 0.0,
                        comment="Polarization angle offset", comfile="NULL",
                        after="", before="", update+, add+, addonly-, delete-,
                        verify-, show-)
                    nhedit (out[i]//"["//k//"]", "T", 1.0,
                        comment="Waveplate transmission", comfile="NULL",
                        after="", before="", update+, add+, addonly-, delete-,
                        verify-, show-)
                    nhedit (out[i]//"["//k//"]", "EPS", 1.0, 
                        comment="Waveplate efficiency of detecting \
                        polarization", comfile="NULL", after="", before="",
                ...
                if (keyfound != "") {
                    for (k=1; k <= 4; k=k+1) {
                        nhedit (out[i]//"["//str(k)//"]", "WCSAXES", 2, 
                            comment="Number of WCS axes in the image",
                            comfile="NULL", after="", before="", update+, add+,
                            addonly-, delete-, verify-, show-)
                        nhedit (out[i]//"["//str(k)//"]", "CTYPE1", keyfound,
                ...
                if (keyfound != "") {
                    for (k=1; k<=4; k=k+1) {
                        nhedit (out[i]//"["//str(k)//"]", "CRPIX1", keyfound,
                            comment="Ref pix of axis 1", comfile="NULL",
                ...
                if (keyfound != "") {
                    for (k=1; k<=4; k=k+1) {
                        nhedit (out[i]//"["//str(k)//"]", "CRVAL1", keyfound,
                ...
                if (keyfound != "") {
                    for (k=1; k<=4; k=k+1) {
                        nhedit (out[i]//"["//str(k)//"]", "CTYPE2", keyfound,
                ...
                if (keyfound != "") {
                    for (k=1; k<=4; k=k+1) {
                        nhedit (out[i]//"["//str(k)//"]", "CRPIX2", keyfound,
                            comment="Ref pix of axis 2", comfile="NULL",
                            after="", before="", update+, add+, addonly-,
                            delete-, verify-, show-)
                    }
                }
                ...
                if (keyfound != "") {
                    for (k=1; k<=4; k=k+1) {
                        nhedit (out[i]//"["//str(k)//"]", "CRVAL2", keyfound,
                ...
                if (keyfound != "") {
                    for (k=1; k<=4; k=k+1) {
                        nhedit (out[i]//"["//str(k)//"]", "CD1_1", keyfound,
                            comment="WCS matrix element 1 1", comfile="NULL",
                            after="", before="", update+, add+, addonly-,
                            delete-, verify-, show-)
                    }
                }
                ...
                if (keyfound != "") {
                    for (k=1; k<=4; k=k+1) {
                        nhedit (out[i]//"["//str(k)//"]", "CD1_2", keyfound,
                            comment="WCS matrix element 1 2", comfile="NULL",
                            after="", before="", update+, add+, addonly-,
                            delete-, verify-, show-)
                    }
                }
                ...
                if (keyfound != "") {
                    for (k=1; k<=4; k=k+1) {
                        nhedit (out[i]//"["//str(k)//"]", "CD2_1", keyfound,
                            comment="WCS matrix element 2 1", comfile="NULL",
                            after="", before="", update+, add+, addonly-,
                            delete-, verify-, show-)
                    }
                }
                ...
                if (keyfound != "") {
                    for (k=1; k<=4; k=k+1) {
                        nhedit (out[i]//"["//str(k)//"]", "CD2_2", keyfound,
                            comment="WCS matrix element 2 2", comfile="NULL",
                            after="", before="", update+, add+, addonly-,
                            delete-, verify-, show-)
                    }
                }
                ...
                if (keyfound != "") {
                    for (k=1; k<=4; k=k+1) {
                        nhedit (out[i]//"["//str(k)//"]", "RADECSYS", keyfound,
                ...
                # EXTNAME and EXTVER
                for (k=1; k<=4; k=k+1) {
                    nhedit (out[i]//"["//str(k)//"]", "EXTNAME", "SCI", 
                        comment="Extension name", comfile="NULL", after="",
                        before="", update+, add+, addonly-, delete-, verify-,
                        show-)
                    nhedit (out[i]//"["//str(k)//"]", "EXTVER", k,
                        comment="Extension version", comfile="NULL", after="",
                        before="", update+, add+, addonly-, delete-, verify-,
                        show-)
                }
                ...
                # Time stamps
                gemdate ()
                nhedit (out[i]//"[0]", "MIPSTACK", gemdate.outdate,
                    "UT Time stamp for MIPSTACK", comfile="NULL", after="",
                    before="", update+, add+, addonly-, delete-, verify-, show-)
                nhedit (out[i]//"[0]", "GEM-TLM", gemdate.outdate,
                    "UT Last modification with GEMINI", comfile="NULL",
                    after="", before="", update+, add+, addonly-, delete-,
                    verify-, show-)
            }
        }
...
clean:
    scanfile = ""
    imdelete ("@"//tmpcomp4, ver-, >& "dev$null")
    imdelete ("@"//tmpcomp3, ver-, >& "dev$null")
    imdelete ("@"//tmpcomp2, ver-, >& "dev$null")
    imdelete ("@"//tmpcomp1, ver-, >& "dev$null")
    delete (tmpfile//","//tmphead, ver-, >& "dev$null")
    delete ("tmpin*", ver-, >& "dev$null")
    delete ("tmpfinal*", ver-, >& "dev$null")
    delete ("tmpfile*", ver-, >& "dev$null")
    delete ("tmpcomp*", ver-, >& "dev$null")
    delete("tmpregister*",ver-, >& "dev$null")
