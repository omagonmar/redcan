    ...
    while (l <= nimages) {
        ...
        if (j != 0) {
            glogprint( l_logfile, "tview", "status", type="error", errno=123,
                str="Image "//l_image//" does not have the expected structure.",
                verbose+)
            status=1
            goto nextimage
        }
        ...
        glogprint( l_logfile, "tview", "visual", type="visual", vistype="empty",
            verbose=l_verbose )
        glogprint( l_logfile, "tview", "status", type="string",
                        ...
                        if (fscan(imcur,xpos,ypos,wcs,cursinput) != EOF) {
                            ...
                            if (cursinput == "x" || cursinput == "X") {
                                ...
                                if (nbadsets != nbadsetsorig && nbadsets > 0) {
                                    hedit(l_image//"["//str(i)//"]","NBADSET",
                                        nbadsets,add=yes,addonly=no,delete=no,
                                        verify=no,show=l_sh_change,update=yes)
                                    for (j=1; j <= nbadsets; j+=1) {
                                        if (j < 10)
                                            hedit(l_image//"["//str(i)//"]",
                                                "BADSET0"//str(j),
                                                badsetnumber[j],add=yes,
                                                addonly=no,delete=no,verify=no,
                                                show=l_sh_change,update=yes)
                                        else
                                            hedit(l_image//"["//str(i)//"]",
                                                "BADSET"//str(j),
                                                badsetnumber[j],add=yes,
                                                addonly=no,delete=no,
                                                verify=no,show=l_sh_change,
                                                update=yes)
                                    }
                                    if (modeflag == 1 || modeflag == 3) {
                                        hedit(l_image//"["//str(i+beamval)//"]",
                                            "NBADSET",nbadsets,add=yes,
                                            addonly=no,delete=no,verify=no,
                                            show=l_sh_change,update=yes)
                                        for (j=1; j <= nbadsets; j+=1) {
                                            if (j > 9)
...
nextexten:
            npars=0
            if (nbadsets != nbadsetsorig && nbadsets > 0) {
                hedit(l_image//"["//str(i)//"]","NBADSET",nbadsets,add=yes,
                    addonly=no,delete=no,verify=no,show=l_sh_change,update=yes)
                for (j=1; j <= nbadsets; j+=1) {
                    if (j < 10)
                        hedit(l_image//"["//str(i)//"]","BADSET0"//str(j),
                            badsetnumber[j],add=yes,addonly=no,delete=no,
                            verify=no,show=l_sh_change,update=yes)
                    else
                        hedit(l_image//"["//str(i)//"]","BADSET"//str(j),
                            badsetnumber[j],add=yes,addonly=no,delete=no,
                            verify=no,show=l_sh_change,update=yes)
                }
                if (modeflag == 1 || modeflag == 3) {
                    hedit(l_image//"["//str(i+beamval)//"]","NBADSET",nbadsets,
                        add=yes,addonly=no,delete=no,verify=no,show=l_sh_change,
                        update=yes)
                    for (j=1; j <= nbadsets; j+=1) {
                        if (j > 9)
                            hedit(l_image//"["//str(i+beamval)//"]",
                                "BADSET"//str(j),badsetnumber[j],add=yes,
                                addonly=no,delete=no,verify=no,show=l_sh_change,
                                update=yes)
                        else
                            hedit(l_image//"["//str(i+beamval)//"]",
                                "BADSET0"//str(j),badsetnumber[j],add=yes,
                                addonly=no,delete=no,verify=no,show=l_sh_change,
                                update=yes)
                    }
                }
            ...
            if (nbadsets < nbadsetsorig) {
                if (nbadsets == 0)
                    hedit(l_image//"["//str(i)//"]","NBADSET",nbadsets,add=no,
                        addonly=no,delete=yes,verify=no,show=l_sh_change,
                        update=yes)
                for (j=nbadsets+1; j < nbadsetsorig; j+=1) {
                    if (j > 9)
                        hedit(l_image//"["//str(i)//"]","BADSET"//str(j),
                            badsetnumber[j],add=no,addonly=no,delete=yes,
                            verify=no,show=l_sh_change,update=yes)
                    else
                        hedit(l_image//"["//str(i)//"]","BADSET0"//str(j),
                            badsetnumber[j],add=no,addonly=no,delete=yes,
                ...
                if (modeflag == 1 || modeflag == 3) {
                    if (nbadsets == 0)
                        hedit(l_image//"["//str(i+beamval)//"]","NBADSET",
                            nbadsets,add=no,addonly=no,delete=yes,verify=no,
                            show=l_sh_change,update=yes)
                    for (j=nbadsets+1; j < nbadsetsorig; j+=1) {
                        if (j > 9)
                            hedit(l_image//"["//str(i+beamval)//"]",
                                "BADSET"//str(j),badsetnumber[j],add=no,
                                addonly=no,delete=yes,verify=no,
                                show=l_sh_change,update=yes)
                        else
                            hedit(l_image//"["//str(i+beamval)//"]",
                                "BADSET0"//str(j),badsetnumber[j],add=no,
                                addonly=no,delete=yes,verify=no,
                                show=l_sh_change,update=yes)
                    }
                }
