    ...
    while (i <= nimages) {
        ...
        if (l_fl_register == no) {
            ...
            xregister (in[i]//"[2]", in[i]//"[1]", regions=l_regions,
                shifts=tmpfile, output=tmpim4, background="none",
            ...
            xregister (in[i]//"[3]", in[i]//"[1]", regions=l_regions,
                shifts=tmpfile, output=tmpim5, background="none",
                loreject=INDEF, hireject=INDEF, apodize=0., filter="none",
                append+, records="", correlation="discrete", xwindow=11,
                ywindow=11, xcbox=11, ycbox=11, function="centroid",
                interp_type="poly5", interact-, xlag=0, ylag=0, dxlag=0,
            ...
            xregister (in[i]//"[4]", in[i]//"[1]", regions=l_regions,
                shifts=tmpfile, output=tmpim6, background="none",
                loreject=INDEF, hireject=INDEF, apodize=0., filter="none",
                append+, records="", correlation="discrete", xwindow=11,
                ywindow=11, xcbox=11, ycbox=11, function="centroid",
                interp_type="poly5", interact-, xlag=0, ylag=0, dxlag=0,
        ...
        fxinsert (tmpim2, out[i]//"[1]", "", verbose=l_verbose)
        fxinsert (tmpim1, out[i]//"[2]", "", verbose=l_verbose)
        ...
        if (l_fl_mask) {
            ...
            imcopy (tmpim3, tmpmask, ver-, >& "dev$null")
            if (l_fl_negative) {
            ...
            imrename (tmpimage, tmpmask, ver-, >& "dev$null")

            nmisc.fixpix (tmpim1, tmpmask, creplace="1-10", linterp="",
                cinterp="", constant=0.)
            nmisc.fixpix (tmpim2, tmpmask, creplace="1-10", linterp="",
                cinterp="", constant=0.)
        }
        ...
        if (l_fl_mask) {
            nmisc.fixpix (tmpim4, tmpmask, creplace="1-10", linterp="",
                cinterp="", constant=0.)
        }
        ...
        imexpr ("deg(atan2(a,b))/2.", tmpim5, tmpim2, tmpim1, dims="auto",
            ...
            verbose-, exprdb="none")
        if (l_fl_mask) {
            nmisc.fixpix (tmpim5, tmpmask, creplace="1-10", linterp="",
                cinterp="", constant=0.)
        }
        ...
        imarith (tmpim3, "-", tmpim4, tmpim6,
            ...
            verbose=no, noact=no)
        if (l_fl_mask) {
            nmisc.fixpix (tmpim6, tmpmask, creplace="1-10", linterp="",
                cinterp="", constant=0.)
        }
        ...
        fxinsert (tmpim6, out[i]//"[3]", "", verbose=l_verbose)
        fxinsert (tmpim4, out[i]//"[4]", "", verbose=l_verbose)
        fxinsert (tmpim7, out[i]//"[5]", "", verbose=l_verbose)
        fxinsert (tmpim5, out[i]//"[6]", "", verbose=l_verbose)
        
        # We're done with this image, delete all the tmp files
        imdelete (tmpim1//","//tmpim2//","//tmpim3//","//tmpim4//","//\
            tmpim5//","//tmpim6//","//tmpim7, ver-, >& "dev$null")
        imdelete (tmpmask, ver-, >& "dev$null")

        # Identify the extensions
        nhedit (out[i]//"[1]", "POLRES", "I Stokes Parameter",
            "Polarization result", comfile="NULL", after="",
            before="", update+, add+, addonly-, delete-, verify-, show-)
        nhedit (out[i]//"[2]", "POLRES", "U Stokes Parameter",
            "Polarization result", comfile="NULL", after="",
            before="", update+, add+, addonly-, delete-, verify-, show-)
        nhedit (out[i]//"[3]", "POLRES", "Q Stokes Parameter",
            "Polarization result", comfile="NULL", after="",
            before="", update+, add+, addonly-, delete-, verify-, show-)
        nhedit (out[i]//"[4]", "POLRES", "Unpolarized Intensity",
            "Polarization result", comfile="NULL", after="",
            before="", update+, add+, addonly-, delete-, verify-, show-)
        nhedit (out[i]//"[5]", "POLRES", "Ip Component",
            "Polarization result", comfile="NULL", after="",
            before="", update+, add+, addonly-, delete-, verify-, show-)
        nhedit (out[i]//"[6]", "POLRES", "Fractional Pol. Value in %",
            "Polarization result", comfile="NULL", after="",
            before="", update+, add+, addonly-, delete-, verify-, show-)
        nhedit (out[i]//"[7]", "POLRES", "Pol. Angle in degrees",
            "Polarization result", comfile="NULL", after="",
            before="", update+, add+, addonly-, delete-, verify-, show-)
        
        # EXTNAME and EXTVER
        for (j=1; j<=7; j=j+1) {
            nhedit (out[i]//"["//j//"]", "EXTNAME", "SCI", "Extension name",
                comfile="NULL", after="", before="", update+, add+, addonly-,
                delete-, verify-, show-)
            nhedit (out[i]//"["//j//"]", "EXTVER", j, "Extension version",
                comfile="NULL", after="", before="", update+, add+, addonly-,
                delete-, verify-, show-)
        }
        ...
        # Time stamps
        gemdate()
        nhedit (out[i]//"[0]", "MIPQL", gemdate.outdate,
            "UT Time stamp for MIPQL", comfile="NULL", after="",
            before="", update+, add+, addonly-, delete-, verify-, show-)
